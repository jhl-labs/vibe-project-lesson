...
> Express + Jest 기반 Clean Architecture — Claude Code로 처음부터 구현하기
...
> Express + Jest + TypeScript 프로젝트를 초기화해줘.
...
<Callout type="tip" title="초기화 팁">
  ...
  "CLAUDE.md를 먼저 생성해줘. 기술 스택은 Express + Jest이고, Clean Architecture를 따르며, Conventional Commits를 사용한다고 적어줘."
</Callout>
...
<Callout type="info" title="Domain Layer 핵심 원칙">
  ...
  - **reconstitute()**: 저장소에서 복원할 때 사용 (검증 없이 기존 데이터로 재구성)
</Callout>
...
<CodeBlock
  code={`// src/application/user/get-user.ts
import { IUserRepository } from '../../domain/user/repository';
...
export class GetUserUseCase {
  ...
  language="typescript"
  filename="src/application/user/get-user.ts"
/>
...
<CodeBlock
  code={`// src/infrastructure/persistence/in-memory-user-repository.ts
import { User } from '../../domain/user/entity';
import { IUserRepository } from '../../domain/user/repository';
...
export class InMemoryUserRepository implements IUserRepository {
  ...
  language="typescript"
  filename="src/infrastructure/persistence/in-memory-user-repository.ts"
  highlightLines={[6, 9, 13, 20]}
/>

<Callout type="info" title="의존성 역전 원칙 (DIP)">
  - Domain Layer가 `IUserRepository` **인터페이스**를 정의
  - Infrastructure Layer가 `InMemoryUserRepository`로 **구현**
  - Application Layer는 인터페이스에만 의존 → 저장소 변경 시 구현체만 교체하면 됨
  - 테스트 시 Mock Repository를 주입하여 DB 없이 단위 테스트 가능
</Callout>

<Callout type="tip" title="프로덕션에서는?">
  InMemoryRepository는 학습용입니다. 프로덕션 환경에서는 Prisma, TypeORM 등 실제 DB 구현체로 교체하면 됩니다. **인터페이스만 동일하게 구현**하면 Application Layer 코드 변경 없이 저장소를 교체할 수 있습니다 — 이것이 의존성 역전의 실제 효과입니다.
</Callout>
...
>    InMemoryUserRepository를 사용해서 의존성을 조립해.
>
> 2. 통합 테스트를 작성해줘:
