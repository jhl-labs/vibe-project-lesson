...
`Shift+Tab`으로 Plan Mode에 진입합니다. Plan Mode에서는 Claude가 파일을 수정하지 않고 **읽기와 계획만** 수행합니다.
...
<Callout type="info" title="Plan Mode 활용 팁">
  - **think hard**: 프롬프트에 "think hard"를 포함하면 Claude가 확장 사고 모드로 더 깊이 분석합니다
  - **계획 저장**: 복잡한 기능은 계획을 `.agent/tasks/` 파일로 저장하면 세션 간 참조 가능
...
<Callout type="warning" title="검증 수단을 항상 포함하세요">
  ...
  Anthropic 공식 베스트 프랙티스에서 이를 **가장 효과적인 단일 기법**으로 권장합니다.
</Callout>
...
export class UploadAvatarUseCase {
  ...
  highlightLines={[6, 7, 18, 21, 26, 27, 35, 36, 41, 42]}
/>
...
# 2단계: 설계 (Shift+Tab으로 Plan Mode 진입)
> 다음 요구사항으로 구현 계획을 세워줘:
> - 현재 비밀번호 확인 후 변경
> - bcrypt로 해싱
