...
<Meta title="Part 1: 시작하기/Claude Code란?" />
...
<Callout type="info" title="핵심 포인트">
  ...
  2025년 한 해 동안 **수백 회의 버전 업데이트**가 배포되었습니다 ([npm registry](https://www.npmjs.com/package/@anthropic-ai/claude-code) 기준).
</Callout>
...
# API 키를 영구 설정하려면 셸 프로필(~/.zshrc, ~/.bashrc 등)에 export를 추가
echo 'export ANTHROPIC_API_KEY="sk-ant-..."' >> ~/.zshrc

# 설정 목록 확인 (인증/모델 상태는 대화 중 /status 슬래시 커맨드)
claude config list`}
  language="bash"
  filename="인증 설정"
/>
...
<Callout type="tip" title="자동 승인 설정">
  ...
  보다 안전한 중간 옵션으로 **Auto mode**가 있습니다. 분류기가 권한 프롬프트를 처리하여 안전한 작업은 자동 실행하고 위험한 작업은 차단하며, hard deny 규칙도 지원합니다. 2026년 6월 기준 Pro 플랜에서도 사용할 수 있습니다.
</Callout>
...
|------|------|-------------------|-----------|
| **Pro** | $20/월 | 기본 (45 메시지/5시간) | Sonnet 4.6 |
| **Max 5x** | $100/월 | Pro의 5배 | Sonnet 4.6 + Opus 4.8 |
| **Max 20x** | $200/월 | Pro의 20배 | Sonnet 4.6 + Opus 4.8 |
| **Teams Standard** | $25/월/석 (연간) | 팀 관리 + 기본 사용량 | Sonnet 4.6 |
| **Teams Premium** | $150/월/석 (연간) | 팀 관리 + Claude Code 포함 + 대량 사용 | 전체 모델 |
| **API** | 종량제 | 무제한 (비용 비례) | 전체 모델 |

<Callout type="tip" title="비용 참고">
  [Anthropic 공식 문서](https://code.claude.com/docs/en/costs)에 따르면 Claude Code 사용자의 평균 비용은 **하루 $6**, 90%의 개발자가 **하루 $12 이하**입니다.
  Max/Team 구독은 사용량이 많을수록 API 종량제 대비 비용 효율적입니다.
</Callout>
...
Claude Code 2.1.x부터 슬래시 커맨드와 스킬이 **통합**되었습니다(2026년 6월 기준 최신 안정 버전 v2.1.162). `.claude/commands/` 또는 `.claude/skills/` 디렉토리에 마크다운 파일을 작성하면 `/명령어`로 호출할 수 있습니다.

| 유형 | 설명 | 예시 |
|------|------|------|
| **내장 커맨드** | Claude Code 기본 제공 | `/init`, `/clear`, `/compact`, `/cost`, `/usage`, `/hooks`, `/config`, `/goal`, `/code-review`, `/recap`, `claude agents` |
...
특정 이벤트에 반응하는 자동화 스크립트입니다. 14개 이벤트 타입과 3가지 핸들러(command, prompt, agent)를 지원합니다:
...
- **실증 사례**: 당시 Opus 4.6 기반의 16개 에이전트가 약 2,000회의 Claude Code 세션(약 $20,000 비용)에 걸쳐 **Rust 기반 C 컴파일러**(약 100,000줄)를 자율 개발했고, 이 컴파일러로 Linux 6.9를 x86/ARM/RISC-V에서 빌드하는 데 성공했습니다 ([Anthropic 엔지니어링 블로그](https://www.anthropic.com/engineering/building-c-compiler), Nicholas Carlini, 2026.02)
...
### 9. Code Review (2026년 3월~)

2026년 3월 9일 리서치 프리뷰로 출시된 **멀티 에이전트 PR 코드 리뷰** 시스템입니다. PR이 열리면 여러 에이전트가 병렬로 버그를 탐지하고, 오탐을 필터링한 뒤 심각도순으로 정렬합니다.

- **멀티 에이전트 병렬 분석**: PR 복잡도에 따라 에이전트 수와 분석 깊이를 자동 조절
- **로직 에러 중심**: 단순 스타일 검사가 아닌 로직 버그, 보안 취약점에 집중
- **자동 검증**: 발견된 버그를 검증 에이전트가 오탐 필터링
- **슬래시 커맨드**: CLI에서 `/code-review`로 실행하면 correctness 버그를 보고하며, `--fix`로 수정을 바로 적용할 수 있습니다(이전 `/simplify`에서 이름 변경). 더 깊은 리뷰가 필요하면 `claude ultrareview` 또는 `/ultrareview`로 클라우드에서 버그 헌팅 에이전트 fleet을 실행하고, 결과를 CLI/Desktop으로 자동 전달받을 수 있습니다(CI/스크립트에서도 사용 가능)
- **비용**: 리뷰당 평균 $15-$25, 약 20분 소요
- **효과**: Anthropic 내부 데이터에서 PR의 54%가 실질적인 리뷰 코멘트를 받았으며(기존 16%), 엔지니어가 오류로 표시한 발견은 1% 미만이었습니다 ([Anthropic Code Review 문서](https://code.claude.com/docs/en/code-review))
- 현재 Team 및 Enterprise 플랜에서 사용 가능 ([Anthropic](https://claude.com/blog/code-review))
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '/effort', values: ['추론 강도 제어 (low/medium/high/xhigh)', 'Opus 4.8 기본값은 high, 최고 난도 작업은 xhigh'] },
...
<ComparisonTable
  title="AI 코딩 도구 상세 비교 (2026년 6월 기준)"
  ...
  rows={[
    ...
    { feature: '프로젝트 컨텍스트', values: ['CLAUDE.md', '.github/copilot-instructions.md', '.cursor/rules/', '.windsurf/rules/', 'CONVENTIONS.md'] },
...
**Claude Code**의 강점:
- **깊은 추론 능력**: Opus 4.8 모델의 추론 성능을 최대한 활용하는 에이전트 아키텍처
...
<MermaidDiagram
  ...
  `}
  title="토큰 구성과 비용 구조"
  caption="입력 토큰보다 출력 토큰의 단가가 5배 높습니다 (Sonnet 4.6 기준: 입력 $3/백만, 출력 $15/백만)"
/>

### 모델별 비용 비교

| 모델 | 입력 토큰 (백만당) | 출력 토큰 (백만당) | 컨텍스트 윈도우 | 최대 출력 | 적합한 작업 |
|------|-------------------|-------------------|----------------|----------|------------|
| **Opus 4.8** | $5 | $25 | 1M 토큰 | 128K | 현재 기본/최강 모델, 복잡한 추론·아키텍처 |
| **Sonnet 4.6** | $3 | $15 | 1M 토큰 | 64K | 일상적 코딩, 속도·지능 균형 |
| **Haiku 4.5** | $1 | $5 | 200K | 64K | 간단한 작업, 대량 처리 |
...
일반적인 작업별 예상 토큰 사용량입니다 (Sonnet 4.6 기준):
...
<Callout type="tip" title="비용 절약 팁">
  **Pro 플랜($20/월)** 사용자는 Sonnet 4.6 기준 5시간당 약 45 메시지를 사용할 수 있습니다. 이 한도를 최대한 활용하려면: (1) 한 번에 명확하고 구체적인 요청을 하고, (2) 간단한 작업은 여러 개를 묶어서 요청하며, (3) 컨텍스트가 커질수록 `/compact`를 적극 활용하세요. **Max 5x($100/월)** 는 사용량이 많을수록 API 종량제 대비 비용 효율이 높아, 하루 평균 $6 이상 사용하는 개발자에게 경제적입니다.
</Callout>
...
| **Windows** | Windows 10 1809+ (네이티브, WSL 불필요). Git for Windows(Git Bash)도 더 이상 필수가 아니며, Bash가 없으면 PowerShell이 shell 도구로 사용됩니다 |

<ChapterNav
  prev={{ title: '바이브 코딩이란?', path: '/docs/part-1--시작하기-바이브-코딩이란' }}
  next={{ title: '개발 환경 설정', path: '/docs/part-1--시작하기-개발-환경-설정' }}
/>
