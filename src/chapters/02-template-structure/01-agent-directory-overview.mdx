...
<CodeBlock
  code={`# CLAUDE.md (프로젝트 루트 또는 .claude/CLAUDE.md)
...
<Callout type="info" title="@ 임포트 구문">
  CLAUDE.md에서 `@path/to/file.md` 구문으로 외부 파일을 임포트할 수 있습니다.
...
<Callout type="tip" title="CLAUDE.md vs .claude/rules/">
  CLAUDE.md에 모든 규칙을 넣으면 컨텍스트 윈도우에서 핵심 정보가 묻히는 현상(fading memory)이 발생합니다.
  핵심 규칙만 CLAUDE.md에 두고, 상세 내용은 `@.agent/context.md`로 임포트하거나
  `.claude/rules/` 디렉토리에 주제별 규칙 파일로 분리하세요.
...
| **GitHub Copilot** | `.github/copilot-instructions.md` | `.github/instructions/*.instructions.md`로 path-specific 규칙도 지원 |
| **Cursor** | `.cursor/rules/*.mdc` | 규칙 내 `@file`로 직접 참조 |

<Callout type="tip" title="크로스 도구 표준: AGENTS.md">
  2025년부터 OpenAI Codex, Cursor, Google Jules 등 여러 AI 도구가 `AGENTS.md` 파일을 공통으로 인식하기 시작했습니다.
  Linux Foundation 산하 Agentic AI Foundation이 관리하는 크로스 도구 표준으로,
  도구 독립적인 에이전트 지시사항을 마크다운으로 관리합니다.
  프로젝트에서 여러 AI 도구를 병행한다면 `AGENTS.md`도 함께 관리하는 것을 고려하세요.
</Callout>
