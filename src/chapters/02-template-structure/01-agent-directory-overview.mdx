...
<ComparisonTable
  ...
  rows={[
    { feature: '설정 포맷', values: ['Markdown (.md)', 'Markdown + YAML (.mdc)', 'Markdown (.md)', '단일 텍스트 파일', 'Markdown (.md) + YAML'] },
    { feature: '메타데이터 지원', values: ['YAML frontmatter (rules)', 'description, globs, alwaysApply', '파일명 기반 (.instructions.md)', '없음', 'YAML frontmatter'] },
    { feature: '범위 지정 (Scoping)', values: ['paths 필드로 glob 패턴', 'globs 필드로 파일 매칭', '파일명에 경로 인코딩', '프로젝트 전체', 'glob 패턴 지원'] },
    { feature: '외부 참조', values: ['@path 구문으로 import', '@file 구문', '직접 참조 불가', '직접 참조 불가', '@file 구문'] },
    { feature: '조건부 로드', values: ['path-scoped로 조건부', 'alwaysApply: false로 조건부', '없음 (항상 전체 로드)', '없음', '조건부 지원'] },
    { feature: '계층 구조', values: ['조직 > 프로젝트 > 사용자 > 로컬', '프로젝트 단위', '조직 > 저장소', '프로젝트 단위', '프로젝트 단위'] },
    { feature: '개인 설정 분리', values: ['CLAUDE.local.md', '없음 (수동 .gitignore)', '없음', '없음', '.roo/local.md'] },
  ]}
/>
...
| 항목 | 내용 |
|------|------|
| **관리 주체** | Linux Foundation 산하 Agentic AI Foundation (2025년 12월 설립, [Announcement](https://linuxfoundation.org/press/agentic-ai-foundation)) |
...
<Callout type="info" title="GitHub Copilot의 instructions 파일">
  `.github/instructions/` 디렉토리의 파일은 파일명에 경로를 인코딩하여 범위를 지정합니다.
  예를 들어 `react.instructions.md`는 React 관련 작업 시 참조됩니다.
  Claude Code의 path-scoped rules와 유사한 개념이지만, 메타데이터가 아닌 파일명 기반이라는 차이가 있습니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    { feature: '프로젝트 규칙', values: ['.claude/rules/*.md', '.cursor/rules/*.mdc', '.github/copilot-instructions.md', '.roo/rules/*.md'] },
    { feature: '전역 지시', values: ['CLAUDE.md', '.cursor/rules/ (alwaysApply)', '.github/copilot-instructions.md', '.roo/rules/ (global)'] },
    { feature: '범위 지정', values: ['paths: ["src/**"]', 'globs: "src/**"', '파일명 인코딩', 'glob 패턴'] },
    { feature: '개인 설정', values: ['CLAUDE.local.md', '수동 .gitignore', '없음', '.roo/local.md'] },
    { feature: '공통 포맷', values: ['AGENTS.md 지원', 'AGENTS.md 지원', 'AGENTS.md 지원', 'AGENTS.md 미정'] },
  ]}
/>
...
└── .cursorrules              # (Cursor 사용 시) 또는 .cursor/rules/
