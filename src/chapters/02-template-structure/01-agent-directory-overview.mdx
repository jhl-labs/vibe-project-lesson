...
## 이 프로젝트 템플릿의 접근: .agent/ 디렉토리

<Callout type="warning" title=".agent/는 공식 표준이 아닙니다">
  `.agent/` 디렉토리는 어떤 AI 도구의 공식 사양도 아니고, 오픈소스 표준도 아닙니다.
  **이 프로젝트 템플릿이 자체적으로 채택한 컨벤션**으로,
  프로젝트 지식(컨텍스트, 규칙, 아키텍처, 프롬프트)을 마크다운 파일로 정리해두는 디렉토리입니다.
  어떤 AI 도구도 이 디렉토리를 자동으로 읽지 않으며, 각 도구의 공식 경로를 통해 명시적으로 연결해야 합니다.
</Callout>

이 템플릿에서 `.agent/`를 사용하는 이유는 **프로젝트 지식을 도구 독립적인 마크다운으로 한 곳에 정리**하고, 각 도구의 공식 경로에서 이를 참조하는 구조를 만들기 위해서입니다.

<FileTree
  title=".agent/ — 이 템플릿의 프로젝트 지식 베이스 (자체 컨벤션)"
  data={[
    {
      name: '.agent/',
      type: 'folder',
...
## .agent/를 각 도구에 연결하는 방법

`.agent/` 파일을 실제 AI 도구에서 사용하려면, 각 도구의 **공식 메커니즘**으로 연결해야 합니다.

### Claude Code: @import 구문
...
@.agent/architecture.md`}
  language="markdown"
  filename="CLAUDE.md — @import로 .agent/ 파일 연결"
/>
...
- Use Conventional Commits`}
  language="markdown"
  filename=".github/copilot-instructions.md — .agent/ 내용을 직접 작성"
/>
...
@.agent/architecture.md 의 레이어 규칙을 따르세요.
Domain 레이어는 외부 의존성을 가질 수 없습니다.`}
  language="markdown"
  filename=".cursor/rules/architecture.mdc — @file로 .agent/ 참조"
/>

### 크로스 도구: AGENTS.md에 핵심 내용 통합

<CodeBlock
  code={`# AGENTS.md

## Architecture
(architecture.md 핵심 내용)

## Conventions
(conventions.md 핵심 내용)

## Guidelines
(guidelines.md 핵심 내용)`}
  language="markdown"
  filename="AGENTS.md — .agent/ 핵심 내용을 통합"
/>
