...
<ComparisonTable
  ...
  rows={[
    { feature: '단위 테스트', values: ['훅, 유틸 함수', 'Vitest 4.x / Jest 30', '매우 빠름 (ms)', '로직 검증'] },
    ...
    { feature: 'E2E 테스트', values: ['사용자 시나리오', 'Playwright 1.58+ / Cypress', '느림 (s~min)', '전체 흐름'] },
  ]}
/>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'Vanilla Extract', values: ['타입 안전, 제로 런타임', '설정 복잡, 생태계 상대적으로 작음', '성능 중시 대규모 프로젝트'] },
  ]}
/>
...
test.describe('대시보드', () => {
  ...
  test('모바일에서 1열 레이아웃이어야 한다', async ({ page }) => {
    ...
    // 브라우저 computed style은 grid-template-columns를 픽셀 트랙으로 직렬화하므로
    // '1fr'이 아니라 단일 트랙(공백 없는 단일 px 값)인지로 1열 레이아웃을 검증한다.
...
<ComparisonTable
  ...
  rows={[
    { feature: '번들러', values: ['Vite 7.x (Rolldown 기반)', 'webpack, Turbopack', 'Vite: 빠른 HMR, 간단한 설정'] },
    { feature: '테스트', values: ['Vitest 4.x + Testing Library', 'Jest 30 + Testing Library', 'Vitest: Vite 생태계 통합, 빠른 실행'] },
    { feature: 'E2E', values: ['Playwright 1.58+', 'Cypress', 'Playwright: 멀티 브라우저, 에이전트 친화 CLI'] },
    { feature: '상태 관리', values: ['Zustand + TanStack Query', 'Redux Toolkit, Jotai', 'Zustand: 간결한 API, 작은 번들'] },
