...
<ComparisonTable
  ...
  rows={[
    { feature: '성능 (req/sec)', values: ['약 20,000~25,000', '약 43,000~80,000 ([공식 벤치마크](https://fastify.dev/benchmarks/))'] },
...
<Callout type="info" title="이 실습에서 Express를 선택한 이유">
  Clean Architecture에서는 **프레임워크가 세부사항(detail)**입니다. Express든 Fastify든 Presentation Layer에만 영향을 미치고, Domain/Application Layer는 변경되지 않습니다. Express 5.x(2024년 10월 출시)를 선택한 이유는 학습 자료가 가장 풍부하고, 대부분의 Node.js 개발자에게 익숙하기 때문입니다. Express 5는 Promise 기반 미들웨어와 향상된 보안을 제공합니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '최신 버전 (2026.03)', values: ['7.x (Rust-free 아키텍처)', '0.3.x', '1.0.0-beta (v1 로드맵 진행 중)'] },
