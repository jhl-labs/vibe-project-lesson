...
<Callout type="info" title="Domain Layer의 핵심 원칙 3가지">
  ...
  **2. 자기 검증(Self-validating)** -- `Email.create("invalid")`를 호출하면 `ValidationError`를 담은 `Failure`를 반환합니다(예외를 throw하지 않는 Result 패턴). 호출부는 `isFailure()`로 결과를 확인해 잘못된 데이터가 시스템에 유입되는 것을 **타입 레벨에서 차단**합니다.
...
<ComparisonTable
  ...
  rows={[
    { feature: '성능 (req/sec)', values: ['수만 req/sec 수준', '약 43,000~80,000 ([공식 벤치마크](https://fastify.dev/benchmarks/))'] },
    { feature: '생태계', values: ['가장 넓음 (10년+)', '빠르게 성장 중'] },
    { feature: 'TypeScript 지원', values: ['@types/express 필요 (v5부터 개선)', '내장 타입 지원'] },
...
<Callout type="info" title="이 실습에서 Express를 선택한 이유">
  Clean Architecture에서는 **프레임워크가 세부사항(detail)**입니다. Express든 Fastify든 Presentation Layer에만 영향을 미치고, Domain/Application Layer는 변경되지 않습니다. Express 5.x를 선택한 이유는 학습 자료가 가장 풍부하고, 대부분의 Node.js 개발자에게 익숙하기 때문입니다. Express 5는 Promise 기반 미들웨어와 향상된 보안을 제공합니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '최신 버전 (2026.06)', values: ['7.x (Rust-free 아키텍처)', '0.3.x', '1.0.0-beta (v1 로드맵 진행 중)'] },
...
<ChapterNav
  prev={{ title: 'Agent SDK와 자동화', path: '/docs/part-4--확장과-자동화-agent-sdk와-자동화' }}
