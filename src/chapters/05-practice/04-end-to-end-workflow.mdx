...
`Shift+Tab`을 2회 눌러 Plan Mode에 진입하거나, `/plan` 명령을 사용합니다 (v2.1.0+). Plan Mode에서는 Claude가 파일을 수정하지 않고 **읽기와 계획만** 수행합니다.
...
<Callout type="info" title="Plan Mode 활용 팁">
  - **활성화 방법**: `Shift+Tab` 2회, `/plan` 명령, 또는 `--permission-mode plan` 플래그로 진입 ([Docs](https://code.claude.com/docs/en/common-workflows))
