...
위의 6단계 워크플로우를 **Agent Teams**로 수행하면, 독립적인 단계를 병렬로 실행하여 전체 소요 시간을 대폭 줄일 수 있습니다. Agent Teams는 실험적(experimental) 기능으로, 정확한 활성화 방법(플래그/환경 변수)은 버전에 따라 다를 수 있으므로 공식 문서를 참고하세요 ([Docs](https://code.claude.com/docs/en/agent-teams)).
