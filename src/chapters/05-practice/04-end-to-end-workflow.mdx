...
<Callout type="tip" title="컨텍스트 보존 전략">
  ...
  서브에이전트는 독립된 컨텍스트 윈도우에서 실행되며, 완료 후 결과만 요약하여 반환합니다.
  커스텀 서브에이전트를 `.claude/agents/` 디렉토리에 마크다운 파일로 정의할 수도 있습니다 ([Docs](https://code.claude.com/docs/en/sub-agents)).
</Callout>
...
`Shift+Tab`을 눌러 Plan Mode로 전환하거나, `--permission-mode plan` 플래그로 시작합니다. Plan Mode에서는 Claude가 파일을 수정하지 않고 **읽기와 계획만** 수행합니다.
...
# 계획을 승인하면 자동으로 편집(구현) 모드로 전환됩니다 (필요 시 Shift+Tab로 모드 전환)`}
  language="text"
  filename="2단계: Plan Mode에서 설계"
/>

<Callout type="info" title="Plan Mode 활용 팁">
  - **활성화 방법**: `Shift+Tab`로 모드 전환, 또는 `--permission-mode plan` 플래그로 시작 ([Docs](https://code.claude.com/docs/en/common-workflows))
  - **확장 사고(Extended Thinking)**: 기본적으로 **활성화**되어 있으며, `Alt+T`로 토글하거나 설정에서 끌 수 있습니다 ([Anthropic Docs](https://platform.claude.com/docs/en/build-with-claude/extended-thinking))
...
<Callout type="warning" title="검증 수단을 항상 포함하세요">
  각 단계에서 "구현 후 테스트를 실행해줘"를 포함하면 Claude가 **자기 검증 루프(self-verification loop)**를 수행합니다. Anthropic 공식 문서([Common Workflows](https://code.claude.com/docs/en/common-workflows))는 테스트 실행을 통한 검증을 꾸준히 권장합니다.
</Callout>
...
<Callout type="info" title="Writer/Reviewer 패턴">
  ...
  이 패턴은 Anthropic 공식 문서([Common Workflows](https://code.claude.com/docs/en/common-workflows))와 커뮤니티에서 널리 권장됩니다.
</Callout>

<Callout type="tip" title="리뷰 자동화 명령 (2026)">
  수동 프롬프트 대신 전용 리뷰 명령을 사용하면 더 빠릅니다:
  - **`/code-review`**: 변경사항의 correctness 버그를 보고하고, `--fix`로 수정까지 적용합니다. (이전 `/simplify`에서 이름이 변경됨) ([Docs](https://code.claude.com/docs/en/code-review))
  - **`claude ultrareview`** / **`/ultrareview`**: 클라우드에서 버그 헌팅 에이전트 fleet를 실행하고, 결과를 CLI나 Desktop으로 자동 전달합니다. CI/스크립트에서도 호출할 수 있습니다.
</Callout>
...
스타트업에서 3주 안에 SaaS MVP를 Claude Code로 개발하는 전체 과정입니다. 실제 도입 사례를 참고한 예시 시나리오로, 팀 상황에 맞게 조정해 활용할 수 있습니다.
...
Claude Code의 CLI 모드(`claude -p`, 구 Headless Mode)를 활용하면 CI/CD 파이프라인에서 자동으로 코드 리뷰, 테스트 생성, 문서 업데이트를 수행할 수 있습니다.
...
# Claude Code 설치 (권장: 네이티브 바이너리, npm은 deprecated)
...
<Callout type="tip" title="Monitor 도구로 실시간 대응 (2026.04)">
  로그를 수동으로 붙여넣는 대신 **Monitor 도구**를 사용하면 백그라운드 이벤트를 대화로 직접 스트리밍할 수 있습니다.
  로그 파일을 tail하거나 모니터링 이벤트를 구독하고, 새로운 에러가 들어올 때 Claude가 실시간으로 반응하도록 구성할 수 있습니다.
  예를 들어 에러 로그 스트림을 감시하다가 동일 에러가 임계치를 넘으면 즉시 원인 분석과 핫픽스 제안을 받는 흐름을 자동화할 수 있습니다.
</Callout>
...
<Callout type="tip" title="Hooks 활용으로 품질 자동화">
  Part 3에서 배운 Hooks를 설정하면 이 워크플로우가 더 강력해집니다 ([Hooks Docs](https://code.claude.com/docs/en/hooks)):
  - `PostToolUse` + `Edit|Write` → 자동 포맷팅 (Prettier). exit code로 상태 전달
  - `Stop` + `agent` → 매 응답 후 자동 테스트 실행
  - `PreToolUse` + `Bash` → 위험 명령 차단 (exit code 2로 블록). HTTP hooks도 지원하여 외부 URL에 JSON POST 가능

  여기에 더해, **security-guidance 플러그인**(2026.05)을 설치하면 작업 중 Claude의 변경사항을 취약점 관점에서 자동 리뷰해 품질 자동화를 한층 강화할 수 있습니다.
</Callout>
...
위의 6단계 워크플로우를 **Agent Teams**로 수행하면, 독립적인 단계를 병렬로 실행하여 전체 소요 시간을 대폭 줄일 수 있습니다. Agent Teams는 실험적(experimental) 기능으로, 환경 변수 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`을 설정하여 활성화합니다 ([Docs](https://code.claude.com/docs/en/agent-teams)).
...
<Callout type="info" title="Agent Teams vs Headless Mode (CLI)">
  **Agent Teams**: 에이전트들이 자율적으로 협력하고 통신. 각 팀원이 독립 컨텍스트에서 병렬 작업하며, tmux/iTerm2 패널로 실시간 모니터링 가능. 복잡한 상호 의존 작업에 적합.
  **Headless Mode** (`claude -p`): 독립적인 작업을 단순 병렬 실행. `--output-format` 옵션으로 text, json, stream-json 출력 지원. CI/CD, 스크립트 자동화에 적합 ([Docs](https://code.claude.com/docs/en/headless)).
...
<ComparisonTable
  ...
  headers={['사례', '규모', '성과']}
  rows={[
    { feature: 'C 컴파일러 구축 (Anthropic)', values: ['16개 에이전트(당시 2026.02 Claude Opus 4.6 기준), 약 2,000 Claude Code 세션, 약 $20,000 비용', '약 100,000줄 Rust 컴파일러, Linux 6.9를 x86/ARM/RISC-V에서 빌드 ([블로그](https://www.anthropic.com/engineering/building-c-compiler), Nicholas Carlini, 2026.02)'] },
    { feature: 'Ramp (핀테크)', values: ['엔지니어링팀 전사 적용, 50% 주간 채택', '월 100만줄+ AI 코드, 다중 세션 병렬 개발로 대기 시간 제거 ([사례](https://claude.com/customers/ramp))'] },
