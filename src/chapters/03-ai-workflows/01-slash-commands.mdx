...
아래는 자주 사용하는 주요 내장 커맨드입니다. Claude Code는 지속적으로 업데이트되므로, `/help`를 실행하면 현재 버전의 전체 커맨드 목록을 확인할 수 있습니다.
...
| 커맨드 | 설명 | 출력물 |
|--------|------|--------|
| `/review` | 코드 리뷰 (내장 `/review`를 확장) | 리뷰 코멘트 및 제안 |
...
<Callout type="info" title="커스텀 /review vs 내장 /review">
  Claude Code에는 내장 `/review`가 있지만, 커스텀 커맨드로 동일 이름의 `/review`를 정의하면
  커스텀 버전이 우선 실행됩니다. 프로젝트별 리뷰 기준(보안 체크리스트, 컨벤션 검증 등)을 반영할 수 있습니다.
</Callout>
