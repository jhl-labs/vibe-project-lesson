...
스킬은 `.claude/skills/` 디렉토리에 정의된 **실행 가능한 전문 능력**입니다. 각 스킬은 `SKILL.md` 파일을 포함하는 디렉토리로 구성되며, 슬래시 커맨드로 호출하거나 Claude가 자동으로 매칭하여 실행합니다. 각 스킬은 특정 작업에 대한 상세한 절차, 체크리스트, 출력 형식을 정의합니다.
