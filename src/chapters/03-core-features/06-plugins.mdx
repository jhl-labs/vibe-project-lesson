...
<Meta title="Part 3: 핵심 기능/Plugins" />
...
<Callout type="tip" title="2026년 Plugins 워크플로우 개선">
  최근 Claude Code는 플러그인 제작/배포 경험을 크게 단순화했습니다 (출처: [Claude Code What's New](https://code.claude.com/docs/en/release-notes), 2026.05).

  - **`.claude/skills` 자동 로드**: 프로젝트의 `.claude/skills` 디렉토리에 둔 스킬은 마켓플레이스 없이 자동으로 로드됩니다.
  - **`claude plugin init`**: 플러그인 골격(매니페스트, 디렉토리 구조)을 한 번에 스캐폴딩합니다.
  - **`--plugin-dir`의 `.zip` 지원**: 디렉토리뿐 아니라 `.zip` 아카이브도 직접 로드할 수 있습니다.
  - **`--plugin-url`**: 원격 아카이브 URL을 fetch하여 플러그인을 로드합니다.
</Callout>
...
<CodeBlock
  ...
  "repository": "https://github.com/example/team-code-quality",
...
| `version` | string | - | 시맨틱 버저닝 (SemVer) |
| `description` | string | - | 플러그인 설명 |
| `author` | object | - | 작성자 정보 (`name`, `email`, `url` 포함) |
| `license` | string | - | 라이선스 (MIT, Apache-2.0 등) |
| `homepage` | string | - | 플러그인 홈페이지/문서 URL |
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '설치 방법', values: ['수동 파일 배치', 'claude plugin install 또는 --plugin-dir'] },
    { feature: '업데이트', values: ['수동 파일 교체', 'claude plugin update'] },
...
# 마켓플레이스에 등록 후 다른 프로젝트에서 설치
claude plugin install my-review-plugin@my-marketplace`}
  language="bash"
...
<CodeBlock
  code={`# /plugin 메뉴의 Discover 탭에서 마켓플레이스 검색 및 플러그인 설치 가능

# CLI에서 플러그인 설치
claude plugin install team-code-quality@my-marketplace

# 프로젝트 스코프로 설치 (팀 공유)
claude plugin install team-code-quality@my-marketplace --scope project

# /plugin 메뉴에서 설치된 플러그인 관리
/plugin

# 플러그인 비활성화
claude plugin disable team-code-quality

# 플러그인 업데이트
claude plugin update team-code-quality

# 플러그인 제거
claude plugin uninstall team-code-quality`}
  language="bash"
  filename="마켓플레이스 명령어"
/>
...
<Callout type="info" title="공식 플러그인 예시: security-guidance">
  Anthropic은 2026년 5월 **security-guidance** 플러그인을 공개했습니다 (출처: [Claude Code What's New](https://code.claude.com/docs/en/release-notes), 2026.05).
  이 플러그인은 작업 중 Claude가 만든 변경사항을 **취약점 관점에서 리뷰**하여, 코드 작성 흐름 안에서 보안 이슈를 조기에 식별하도록 돕습니다.
  팀 마켓플레이스를 구성할 때 이런 검증된 공식 플러그인을 기본 셋에 포함하는 것을 권장합니다.
</Callout>
...
# 필수 플러그인 설치 (마켓플레이스가 이미 추가된 상태)
REQUIRED_PLUGINS=(
  "team-code-quality@team-marketplace"       # 코드 리뷰, 테스트
  "team-security-tools@team-marketplace"     # 보안 스캔, 시크릿 탐지
  "team-deploy-tools@team-marketplace"       # 배포 워크플로우
  "team-conventions@team-marketplace"        # 코딩 컨벤션 자동 검증
)
...
<CodeBlock
  code={`# 프로젝트별 플러그인 버전 관리 전략
# 마켓플레이스에서 특정 버전을 설치할 수 있습니다
claude plugin install team-code-quality@my-marketplace

# --scope project로 설치하면 .claude/settings.json의
# enabledPlugins에 기록되어 팀원과 공유됩니다
claude plugin install team-code-quality@my-marketplace --scope project
...
| 설치 후 작동 안 함 | 버전 호환성 | Claude Code 최신 안정 버전(2026.06 기준 v2.1.x, 예: 2.1.162)으로 업데이트 후 `claude --version` 확인 |
...
<CodeBlock
  code={`# 플러그인 설치 후 사용
claude plugin install team-review@my-marketplace
...
# 심층 리뷰 에이전트 호출 (서브에이전트)
# 서브에이전트는 슬래시로 직접 호출하지 않습니다.
# (1) description 기반 자동 위임: 아래처럼 자연어로 요청하면
#     deep-reviewer의 description과 매칭되어 Claude가 자동 위임합니다.
"전체 코드베이스를 심층 리뷰해줘"
# (2) 또는 Task 도구로 명시적으로 deep-reviewer 서브에이전트를 지정해 실행합니다.
...
- **`claude plugin install`**: 마켓플레이스에서 설치
- **마이그레이션**: 기존 `.claude/` 설정을 플러그인으로 변환 가능
- **보안**: allowed-tools 최소 권한, 시크릿 환경 변수 참조, 코드 리뷰 필수

<ChapterNav
  prev={{ title: 'Hooks', path: '/docs/part-3--핵심-기능-hooks' }}
  next={{ title: 'MCP 개요', path: '/docs/part-4--확장과-자동화-mcp-개요' }}
/>
