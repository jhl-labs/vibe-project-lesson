...
**OWASP Top 10:2021 체크리스트:** A01 접근 제어 실패(Broken Access Control), A02 암호화 실패(Cryptographic Failures), A03 인젝션(Injection), A04 불안전한 설계(Insecure Design), A05 보안 설정 오류(Security Misconfiguration), A06 취약하고 오래된 컴포넌트(Vulnerable and Outdated Components), A07 인증·식별 실패(Identification and Authentication Failures), A08 소프트웨어·데이터 무결성 실패(Software and Data Integrity Failures), A09 보안 로깅·모니터링 실패(Security Logging and Monitoring Failures), A10 SSRF(Server-Side Request Forgery)
...
model_compatibility:
  - claude-opus-4-8: tested      # 현행 기본 모델
  - claude-sonnet-4-6: tested
  - claude-haiku-4-5: tested
  - claude-opus-4-6: tested      # legacy
  - gpt-4o: untested
success_rate: 87%
avg_iterations: 1.3
---
...
<ComparisonTable
  headers={['버전 변경', '의미', '예시']}
  rows={[
    { feature: 'Major (x.0.0)', values: ['출력 형식이 완전히 변경되어 기존 워크플로우와 호환되지 않음', '출력을 Markdown에서 JSON으로 변경'] },
    { feature: 'Minor (0.x.0)', values: ['새로운 섹션/기능이 추가되었으나 기존 출력은 유지', 'GraphQL 지원 추가, 새로운 체크리스트 항목'] },
    { feature: 'Patch (0.0.x)', values: ['문구 개선, 오타 수정, 예시 보완 등 사소한 변경', '예시 코드 업데이트, 설명 명확화'] },
  ]}
/>
...
<ComparisonTable
  headers={['지표', '설명', '측정 방법', '목표값(예시 권장치)']}
  rows={[
    { feature: '성공률', values: ['AI 응답이 수정 없이 사용 가능한 비율', '수정 없이 사용한 횟수 / 전체 사용 횟수', '85% 이상'] },
    { feature: '반복 횟수', values: ['원하는 결과를 얻기까지 프롬프트를 수정한 횟수', '최종 결과까지의 대화 턴 수', '1.5회 이하'] },
    { feature: '코드 품질', values: ['생성된 코드의 정적 분석 점수', 'ESLint/SonarQube 점수', 'A등급'] },
    { feature: '시간 절감', values: ['프롬프트 사용 전후 작업 시간 비교', '작업 완료 시간 측정', '50% 이상 절감'] },
    { feature: '재작업률', values: ['코드 리뷰에서 수정 요청이 발생한 비율', '리뷰 피드백 건수 / PR 건수', '20% 이하'] },
  ]}
/>
...
<ComparisonTable
  ...
  rows={[
    { feature: 'Zero-shot', values: ['단순한 작업, AI가 이미 잘 아는 영역', '프롬프트가 짧고 간결', '출력 형식이 불안정할 수 있음'] },
    { feature: 'Few-shot', values: ['특정 형식의 출력이 필요할 때', '일관된 출력 형식 보장', '프롬프트 길이가 길어짐 (토큰 소비)'] },
    { feature: 'Chain of Thought', values: ['복잡한 추론이 필요한 분석/설계', '정확도가 높고 사고 과정 검증 가능', '응답 시간이 길어지고 토큰 소비 증가'] },
    { feature: '메타 프롬프트', values: ['새로운 프롬프트를 체계적으로 만들 때', '프롬프트 품질이 일관적', '초기 설정에 시간이 필요'] },
    { feature: '역할 지정', values: ['전문 분야의 깊은 지식이 필요할 때', '도메인 특화 응답', '잘못된 역할은 환각을 유발할 수 있음'] },
    { feature: '제약 기반', values: ['출력의 형식/범위를 엄격히 통제할 때', '예측 가능한 출력', '너무 많은 제약은 창의성을 제한'] },
  ]}
/>
