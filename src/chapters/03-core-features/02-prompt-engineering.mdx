...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '"ultrathink" (스킬 내 사용, deprecated)', values: ['스킬 내에서 extended thinking 활성화 (현재는 기본 활성화됨)', '스킬 본문에 포함하여 깊은 사고 유도'] },
...
<Callout type="info" title="ultrathink/megathink 키워드 변경 사항">
  초기 Claude Code에서는 "think", "megathink", "ultrathink" 키워드로 사고 깊이를 제어했습니다.
  현재는 extended thinking이 기본 활성화되어 있으며, `/model`에서 좌우 화살표로 effort level(low/medium/high)을
  조정하거나 환경변수 `CLAUDE_CODE_EFFORT_LEVEL`로 설정할 수 있습니다. (API에서는 `max` 레벨도 지원)
  다만, 스킬(SKILL.md) 본문에 "ultrathink"을 포함하면 해당 스킬에서
  extended thinking이 활성화되는 기능은 여전히 공식 지원됩니다.
</Callout>
