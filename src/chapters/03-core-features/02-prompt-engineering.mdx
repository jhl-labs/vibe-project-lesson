...
<ComparisonTable
  ...
  rows={[
    { feature: 'Effort Level (/model에서 좌우 화살표 또는 환경변수)', values: ['사고 깊이를 세밀하게 제어 (low/medium/high)', '작업 복잡도에 따라 사고 수준 조정'] },
...
<Callout type="info" title="ultrathink/megathink 키워드 — 최신 현황 (2026.03)">
  ...
  Opus 4.6 모델은 기본적으로 medium effort로 동작하며, `/model`에서 좌우 화살표로 effort level(low/medium/high)을
  조정하거나 환경변수 `CLAUDE_CODE_EFFORT_LEVEL`로 설정할 수 있습니다. (API에서는 `max` 레벨도 지원)
</Callout>
