...
<ComparisonTable
  ...
  rows={[
    { feature: '/effort max (또는 high/medium/low)', values: ['사고 깊이를 세밀하게 제어', '작업 복잡도에 따라 사고 수준 조정'] },
...
<Callout type="info" title="ultrathink/megathink 키워드 변경 사항">
  ...
  현재는 extended thinking이 기본 활성화되어 있으며, `/effort` 커맨드(low/medium/high/max)로
  세밀한 제어가 가능합니다. 다만, 스킬(SKILL.md) 본문에 "ultrathink"을 포함하면 해당 스킬에서
  extended thinking이 활성화되는 기능은 여전히 공식 지원됩니다.
</Callout>

<CodeBlock
  code={`# 사고 깊이 제어 (/effort 커맨드)
/effort max
