...
Claude Code는 터미널에서 자연어로 지시하는 도구입니다. "좋은 코드를 얻는 것"은 "좋은 지시를 내리는 것"에서 시작합니다. [Anthropic 공식 문서](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/overview)에서는 프롬프트 엔지니어링 기법을 **가장 범용적인 것부터 전문적인 것** 순서로 시도할 것을 권장합니다.
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '6', values: ['Give Claude a role (system prompts)', '시스템 프롬프트로 역할 부여'] },
    { feature: '7', values: ['Chain complex prompts', '복잡한 작업을 여러 단계로 분할'] },
    { feature: '8', values: ['Long context tips', '긴 컨텍스트 처리 최적화'] },
  ]}
/>
...
[Anthropic 공식 베스트 프랙티스 문서](https://code.claude.com/docs/en/best-practices)에서 제시하는 Claude Code의 핵심 워크플로우입니다.
...
# 2단계: Plan - 계획 수립 (Shift+Tab 2회 또는 /plan으로 Plan Mode 진입)
...
<Callout type="tip" title="공식 권장: 검증 수단을 함께 제공하세요">
  [Anthropic](https://code.claude.com/docs/en/best-practices)이 제시하는 **단일 최고 레버리지 팁**은 "Claude가 스스로 검증할 수 있게 하라"입니다.
...
<Callout type="warning" title="피해야 할 패턴">
  [Anthropic 공식 문서](https://code.claude.com/docs/en/best-practices)에서 경고하는 일반적 실패 패턴:
...
<ComparisonTable
  ...
  rows={[
    { feature: 'Effort Level (/effort 슬래시 커맨드, /model 좌우 화살표 또는 환경변수)', values: ['사고 깊이를 세밀하게 제어 (low/medium/high/xhigh)', '작업 복잡도에 따라 사고 수준 조정'] },
    { feature: '"think" 키워드', values: ['extended thinking 활성화 (~4K 토큰 예산)', '중간 복잡도 작업에 사고 과정 추가'] },
    { feature: '"megathink" 키워드', values: ['더 깊은 사고 활성화 (~10K 토큰 예산)', '복잡한 분석이 필요한 작업'] },
    { feature: '"ultrathink" / "think harder" / "think really hard" / "think very hard"', values: ['최대 사고 깊이 활성화 (~32K 토큰 예산)', 'v2.1.68에서 복원. 아키텍처 설계, 복잡한 디버깅 등'] },
    { feature: '"간단하게"', values: ['최소 사고로 빠른 응답', '단순한 수정, 보일러플레이트 생성'] },
    { feature: 'Plan Mode (Shift+Tab 2회 또는 /plan)', values: ['코드 수정 없이 분석/계획만', '구현 전 전략 수립, 코드베이스 탐색'] },
  ]}
/>

<Callout type="info" title="ultrathink/megathink 키워드 — 최신 현황 (2026.06)">
  초기 Claude Code에서는 "think", "megathink", "ultrathink" 키워드로 사고 깊이를 제어했습니다.
  한때 ultrathink가 제거되었으나, 커뮤니티 피드백을 반영하여 **v2.1.68에서 ultrathink가 복원**되었습니다.
  각 키워드의 토큰 예산: **think ~4K, megathink ~10K, ultrathink ~32K 토큰**.
  "think harder", "think really hard", "think very hard" 등의 표현도 모두 ultrathink로 매핑됩니다.
  현재 기본 모델인 **Opus 4.8은 모든 표면에서 기본적으로 high effort로 동작**하며, adaptive thinking으로
  작업 난도에 맞춰 사고량을 스스로 조절합니다. `/effort` 슬래시 커맨드로 effort level(low/medium/high/xhigh)을
  지정하거나, `/model`에서 좌우 화살표, 환경변수 `CLAUDE_CODE_EFFORT_LEVEL`로도 설정할 수 있습니다.
  최고 난도 작업에는 `/effort xhigh`를 사용하세요.
</Callout>

<CodeBlock
  code={`# 사고 깊이 제어 (effort level)
# /model 실행 후 좌우 화살표로 effort를 high로 설정
# 또는 환경변수: CLAUDE_CODE_EFFORT_LEVEL=high
"이 마이크로서비스 간 순환 의존성 문제를 분석하고
해결 방안을 제시해줘. 서비스 A→B→C→A 순환이 발생하고 있어"

# Plan Mode 활용 (분석만, 수정 없음)
# Shift+Tab 2회 또는 /plan 으로 Plan Mode 진입 후:
"이 프로젝트의 인증 시스템 전체 흐름을 분석해줘.
로그인부터 토큰 갱신, 로그아웃까지의 플로우를 파악해줘"

# 간단한 작업 (빠른 처리)
# /model 실행 후 좌우 화살표로 effort를 low로 설정
...
현재 기본/최강 모델인 **Opus 4.8**(2026.05.28 출시)은 effort 파라미터가 모든 표면에서 기본 `high`이며,
adaptive thinking으로 작업 난도에 맞춰 사고량을 스스로 조절합니다. 깊은 분석이 필요한 작업에는
`/effort xhigh`로 끌어올리고, 단순 편집에는 `low`/`medium`으로 낮춰 응답 속도와 비용을 최적화하세요.

### 과잉 설계 방지

Opus 4.x 계열(현행 Opus 4.8 포함)은 불필요한 추상화나 추가 파일을 생성하는 경향이 있을 수 있습니다.
CLAUDE.md에 다음을 추가하면 효과적입니다:
...
# Plan Mode 활용 (Shift+Tab 2회 또는 /plan)
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '과도하게 복잡한 코드', values: ['Opus 모델의 과잉 설계 경향', 'CLAUDE.md에 "단순하게 유지" 규칙 추가 또는 Sonnet 4.6 사용'] },
...
<Callout type="tip" title="Custom Slash Commands 활용">
  Claude Code의 `.claude/commands/` 또는 `.claude/skills/` 디렉토리에 자주 사용하는 프롬프트를 저장하면
  `/command-name` 형식으로 즉시 실행할 수 있습니다.
  팀 공유는 Git으로, 개인용은 `~/.claude/commands/` 또는 `~/.claude/skills/`에 저장하세요.
</Callout>
