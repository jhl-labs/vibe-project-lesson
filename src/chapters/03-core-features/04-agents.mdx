...
<ComparisonTable
  ...
  rows={[
    { feature: '코드베이스 탐색 (Explore)', values: ['Haiku', '단순 검색과 패턴 매칭', 'Opus 대비 약 80% 절감'] },
    { feature: '구현 전략 수립 (Plan)', values: ['Sonnet', '분석력과 비용의 균형', 'Opus 대비 약 40% 절감'] },
    { feature: '보안 분석', values: ['Opus', '높은 정확도 필수', '비용 절감 없음 (정확도 우선)'] },
    { feature: '테스트 코드 생성', values: ['Sonnet', '패턴 기반 코드 생성', 'Opus 대비 약 40% 절감'] },
    { feature: '문서 생성', values: ['Sonnet', '충분한 텍스트 생성 능력', 'Opus 대비 약 40% 절감'] },
...
<Callout type="info" title="비용 참고">
  2026년 2월 기준, Opus는 입출력 토큰 모두 Sonnet 대비 약 1.7배, Haiku 대비 약 5배 높은 비용입니다.
...
<Callout type="info" title="출처">
  Nicholas Carlini(Anthropic 안전성 연구원)의 공식 엔지니어링 블로그 포스트:
