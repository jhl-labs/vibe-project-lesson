...
disallowedTools: Write     # 차단할 도구 목록
model: sonnet              # 사용할 모델 (sonnet, opus, haiku, inherit)
permissionMode: default    # 권한 모드 (default, acceptEdits, plan, bypassPermissions 등)
maxTurns: 20               # 최대 에이전틱 턴 수
skills:                    # 시작 시 로드할 스킬
  - api-conventions
memory: project            # 지속적 메모리 (user, project, local)
mcpServers:                # 에이전트 전용 MCP 서버 설정
  - github
  - filesystem
hooks:                     # 에이전트 전용 Hook 설정
  PostToolUse:
    - matcher: "Write"
      hooks:
        - type: command
