...
서브에이전트는 **특정 유형의 작업을 전담하는 전문 AI 어시스턴트**입니다. 각 서브에이전트는 독립된 컨텍스트 윈도우에서 커스텀 시스템 프롬프트, 특정 도구 접근, 독립적 권한으로 실행됩니다. `.claude/agents/` 디렉토리에 YAML 프론트매터가 포함된 마크다운 파일로 역할별 정의를 저장하여 일관된 전문성을 유지합니다. `/agents` 커맨드로 관리하거나 직접 파일을 생성할 수 있습니다. 서브에이전트는 컨텍스트 보존, 도구 제한을 통한 제약 강제, 비용 관리(Haiku 같은 경량 모델 활용) 등에 효과적입니다.
...
|-------------|------|---------------|------|
| **Explore** | 빠른 읽기 전용 코드베이스 탐색·분석 | 읽기 전용 도구 (Write, Edit 불가) | Haiku |
| **Plan** | Plan 모드에서 코드베이스 연구·컨텍스트 수집 | 읽기 전용 도구 (Write, Edit 불가) | inherit |
| **general-purpose** | 탐색과 수정이 모두 필요한 복합 멀티스텝 작업 | 모든 도구 | inherit |
| **Bash** | 별도 컨텍스트에서 터미널 명령 실행 | Bash | inherit |
| **statusline-setup** | /statusline 실행 시 상태줄 스크립트 생성 | 상태줄 설정 전용 | Sonnet |
| **Claude Code Guide** | Claude Code 기능에 대한 질문 응답 | Claude Code 질문 시 사용 | Haiku |
...
<CodeBlock
  code={`---
# 표준 서브에이전트(.claude/agents/*.md) frontmatter 필드
name: agent-name           # 에이전트 식별자 (필수, 소문자+하이픈)
description: 역할 설명      # Claude가 자동 매칭에 사용 (필수)
tools: Read, Glob, Grep    # 사용 가능한 도구 목록 (생략 시 전체 상속)
model: sonnet              # 사용할 모델 (sonnet, opus, haiku, inherit)
# 아래 옵션은 Agent SDK(AgentDefinition/ClaudeAgentOptions)에서 지정하는 항목으로,
# 마크다운 frontmatter가 아니라 SDK 코드로 설정합니다.
# - disallowedTools: 차단할 도구 목록
# - permissionMode: 권한 모드 (default, acceptEdits, plan, bypassPermissions 등)
# - maxTurns: 최대 에이전틱 턴 수
# - mcpServers / hooks: 에이전트 전용 MCP 서버·Hook 설정
skills:                    # 시작 시 로드할 스킬
  - api-conventions
---
...
<Callout type="tip" title="서브에이전트 권한 제어">
  settings.json의 `permissions.deny` 규칙으로 특정 서브에이전트를 차단할 수 있습니다.
  예: `"permissions": { "deny": ["Task(Explore)", "Task(my-custom-agent)"] }`
  CLI 플래그로도 가능합니다: `claude --disallowedTools "Task(Explore)"`
</Callout>
...
# 5. verbose / debug 모드 활용
# claude --verbose 플래그로 실행하면
# 서브에이전트 호출 과정이 더 상세히 출력됩니다
# 더 정밀한 디버깅이 필요하면 CLAUDE_DEBUG=1 환경 변수를 사용합니다`}
  language="bash"
...
<CodeBlock
  code={`## 서브에이전트 성능 지표
# 아래 목표치는 공식 수치가 아닌 경험적 권장 기준입니다 — 팀/프로젝트 특성에 맞게 조정하세요.
...
- 경험상 서브에이전트가 직접 처리 대비 토큰을 과도하게(예: 수 배 이상) 소비한다면 프롬프트 최적화 필요
...
<ComparisonTable
  ...
  headers={['작업 유형', '권장 모델', '이유', '토큰 단가 절감 (Opus 대비)']}
  rows={[
    { feature: '코드베이스 탐색 (Explore)', values: ['Haiku', '단순 검색과 패턴 매칭', '80% 저렴 ($1/$5 vs $5/$25)'] },
    { feature: '구현 전략 수립 (Plan)', values: ['Sonnet', '분석력과 비용의 균형', '40% 저렴 ($3/$15 vs $5/$25)'] },
    { feature: '보안 분석', values: ['Opus', '높은 정확도 필수', '절감 없음 (정확도 우선)'] },
    { feature: '테스트 코드 생성', values: ['Sonnet', '패턴 기반 코드 생성', '40% 저렴 ($3/$15 vs $5/$25)'] },
    { feature: '문서 생성', values: ['Sonnet', '충분한 텍스트 생성 능력', '40% 저렴 ($3/$15 vs $5/$25)'] },
    { feature: '복합 리팩토링', values: ['Opus', '복잡한 의존성 분석 필요', '절감 없음 (품질 우선)'] },
  ]}
/>

<Callout type="info" title="단가 기준">
  위 절감률은 2026년 6월 공식 1M 토큰당 가격(Opus 4.8 $5/$25, Sonnet 4.6 $3/$15, Haiku 4.5 $1/$5)에 근거한 단가 차이입니다. 실제 작업 비용은 모델별 토큰 소비량에 따라 달라집니다 (출처: [Claude 모델 가격](https://platform.claude.com/docs/en/about-claude/models)).
</Callout>
...
<Callout type="info" title="비용 참고">
  2026년 6월 공식 가격(1M 토큰당 input/output)은 Opus 4.8 $5/$25, Sonnet 4.6 $3/$15, Haiku 4.5 $1/$5입니다 (출처: [Claude 모델 가격](https://platform.claude.com/docs/en/about-claude/models)).
  즉 input/output 모두 Opus는 Sonnet 대비 약 1.67배, Haiku 대비 5배 높습니다.
...
2026년 2월 Claude Opus 4.6과 함께 처음 출시된 **Agent Teams**는 서브에이전트의 진화형입니다. 기존 서브에이전트가 "메인 에이전트 → 서브에이전트"의 일방향 위임이었다면, Agent Teams는 **여러 Claude 인스턴스가 팀으로 직접 협력**하는 네이티브 멀티 에이전트 시스템입니다. (2026년 6월 현재 권장 기본 모델은 Opus 4.8이며, Agent Teams의 팀 리드/팀원에도 Opus 4.8을 사용할 수 있습니다.)
...
### Dynamic Workflows — 스크립트로 수십~수백 서브에이전트 오케스트레이션

위의 수동 병렬 실행을 한 단계 더 자동화한 것이 **Dynamic Workflows**입니다 (2026년 5월, Opus 4.8과 함께 도입). 개발자가 직접 `&`로 프로세스를 띄우는 대신, **Claude이 스크립트를 작성해 수십~수백 개의 서브에이전트를 백그라운드에서 오케스트레이션**합니다. 대규모 마이그레이션이나 코드베이스 전반 스캔처럼 동질적인 작업이 매우 많을 때, 작업을 잘게 나눠 대량의 서브에이전트로 병렬 처리하고 결과를 취합합니다.

<Callout type="tip" title="언제 Dynamic Workflows를 쓰는가">
  Agent Teams가 "소수의 팀원이 서로 통신하며 협업"하는 데 적합하다면, Dynamic Workflows는 "독립적인 동질 작업이 수십~수백 건"일 때 적합합니다.
  예: 모든 파일에 대한 일괄 패턴 치환, 수백 개 테스트의 분산 실행, 대규모 deprecated API 마이그레이션.
  Claude이 스크립트로 분배·실행·취합을 관리하므로 사람이 프로세스를 일일이 띄울 필요가 없습니다.
</Callout>

### Agent View — 모든 세션 한눈에 보기 (`claude agents`)

병렬 서브에이전트와 백그라운드 작업이 많아지면 진행 상황 파악이 어려워집니다. **Agent View**(`claude agents`, 2026년 5월 도입)는 실행 중인 모든 Claude Code 세션을 **한 화면에서** 보여줍니다. 각 세션의 상태(실행 중 / 대기 / 완료)를 한눈에 확인하고, 필요한 세션으로 전환할 수 있습니다.

<CodeBlock
  code={`# 실행 중인 모든 Claude Code 세션을 한 화면에서 확인
claude agents
...
# - 세션 간 빠른 전환`}
  language="bash"
  filename="Agent View (claude agents)"
/>
...
| **에이전트 수** | 16개 인스턴스 (당시 최신 모델인 Claude Opus 4.6 사용, 2026.02) |
...
<Callout type="info" title="출처">
  Nicholas Carlini(Anthropic 안전성 연구원)의 공식 엔지니어링 블로그 포스트 (2026.02): [Building a C compiler with Claude](https://www.anthropic.com/engineering/building-c-compiler)
  당시 최신 모델이던 Opus 4.6으로 수행된 역사적 사례이며, 16개 에이전트·약 2,000회 Claude Code 세션·약 $20,000 비용·약 100,000줄 컴파일러 코드, 그리고 GCC torture test 약 99% 통과·Doom 컴파일 및 실행 등의 검증 결과는 모두 이 블로그 글에 근거합니다.
</Callout>
...
최소한의 사람 개입으로 에이전트들이 대부분 자율적으로 개발을 진행했습니다. 저자인 Carlini는 설정 후 "대부분 자리를 비웠다(mostly walked away)"고 밝혔으며, 테스트 인프라 설계와 진행 모니터링 정도만 관여했습니다.
...
지금까지 다룬 서브에이전트, Agent Teams, Mailbox 메시징, Agent SDK의 관계를 정리하면 다음과 같습니다.
...
**Agent Teams**는 2026년 2월 5일 Claude Opus 4.6과 함께 처음 출시된 Claude Code의 **네이티브 멀티 에이전트 협업 기능**입니다. 이후 모델 세대가 갱신되어, 2026년 6월 현재는 Opus 4.8을 팀 리드/팀원에 사용할 수 있습니다. 기존 서브에이전트가 메인 에이전트에서 서브에이전트로의 일방향 위임이었다면, Agent Teams는 하나의 **팀 리드(Team Lead)**가 여러 **팀원(Teammates)**을 생성하고 조율하며, 팀원들이 서로 직접 메시지를 교환하는 완전한 멀티 에이전트 시스템입니다.
...
| **파일 잠금 (File Locking)** | 여러 팀원이 동시에 같은 작업을 클레이밍하는 경쟁 조건을 방지하는 메커니즘 |
...
팀원이 유휴 상태에 들어가려 할 때 발생합니다. exit code 2로 종료하면 피드백 메시지를 팀원에게 보내고 계속 작업하게 만들 수 있습니다.
...
# 유휴 팀원 감지 시 피드백을 보내 작업 계속 지시

INPUT=$(cat)

# 로그 출력
echo "[Agent Teams] 팀원이 유휴 상태에 진입하려 합니다." >&2

# exit 2: 피드백 메시지를 팀원에게 전달하고 유휴 진입을 방지
# exit 0: 유휴 상태 진입 허용
echo "남은 작업이 있는지 작업 목록을 다시 확인하고, 테스트가 모두 통과하는지 검증하세요." >&2
exit 2`}
  language="bash"
...
작업이 완료 상태로 표시되려 할 때 발생합니다. exit code 2로 종료하면 완료를 막고 피드백을 전달하여 추가 작업을 요청할 수 있습니다.
...
# 작업 완료 표시 전 품질 게이트 역할
# 주의: 아래는 통과(exit 0)형 기본 예시입니다. 실제로 완료를 "차단"하려면
#       조건 검사 후 exit 2를 반환해야 합니다(아래 주석 참고).

INPUT=$(cat)

echo "[TaskCompleted] 작업이 완료 처리되려 합니다." >&2

# exit 0: 완료 허용
# exit 2: 완료를 막고 stderr 메시지를 피드백으로 전달
# 예: 테스트 실패 시 차단하려면 → if ! npm test; then echo "테스트 실패" >&2; exit 2; fi
echo "작업을 완료하기 전에 관련 테스트가 모두 통과하는지 확인하세요." >&2
exit 0`}
  language="bash"
...
### 비용 추정 공식 (개념적 — 실제 값은 토큰 소비량에 따라 달라짐)
총 비용 ≈ (팀 리드 비용) + (팀원 수 x 평균 팀원 비용) + (메시지 오버헤드)

### 단가 기준 (2026.06 공식 가격, 1M 토큰당 input/output)
- Opus 4.8: $5 / $25
- Sonnet 4.6: $3 / $15
- Haiku 4.5: $1 / $5
- (출처: platform.claude.com/docs/en/about-claude/models)

### 핵심 직관 (구체 금액 아님)
- 팀원 N명이 각각 독립 세션으로 토큰을 소비하므로, 단일 에이전트 대비 총 토큰이 수 배로 늘어날 수 있음
- 팀원 간 메시지와 작업 목록 동기화로 추가 토큰 오버헤드 발생
- 따라서 핵심 구현만 Opus, 탐색/테스트는 Sonnet으로 차등 배치하는 것이 비용에 가장 큰 영향`}
  language="markdown"
  filename="비용 구조"
/>
...
<ComparisonTable
  title="비용 최적화 전략 (절감폭은 대략적 추정 — 작업 특성에 따라 달라짐)"
  headers={['전략', '방법', '대략적 절감폭']}
  rows={[
    { feature: '모델 차등 배치', values: ['탐색은 Sonnet, 핵심 구현만 Opus', '큼 (단가 차이 직접 반영)'] },
    { feature: '팀 규모 최소화', values: ['필요한 최소 인원으로 팀 구성', '팀원 1명당 비용 비례 절감'] },
    { feature: '작업 범위 한정', values: ['각 팀원의 작업 범위를 구체적으로 지정', '중간 (불필요 토큰 감소)'] },
    { feature: 'Plan Approval 활용', values: ['잘못된 방향 조기 차단으로 재작업 방지', '작음~중간 (재작업 방지분)'] },
    { feature: '서브에이전트와 혼용', values: ['독립 작업은 서브에이전트, 협업만 Agent Teams', '큼 (불필요한 팀 세션 제거)'] },
  ]}
/>
...
<Callout type="warning" title="비용 주의">
  Agent Teams는 팀원이 각각 독립 세션으로 실행되므로, 단일 에이전트 대비 총 토큰 소비가 수 배 수준으로 늘어날 수 있습니다(정확한 배수는 팀 규모와 작업량에 따라 다름). 단순한 작업에 Agent Teams를 사용하는 것은 비용 낭비입니다. 반드시 팀원 간 실시간 소통이 필요한 복합 작업에만 사용하세요.
</Callout>
...
| 제한사항 | 설명 | 완화 방법 |
|----------|------|----------|
| **세션 재개 불가** | in-process 모드에서 /resume, /rewind로 팀원 복원 불가 | 중요 결과는 파일로 저장, 작업 분할을 작게 유지 |
| **작업 상태 지연** | 팀원이 작업을 완료로 표시하지 않아 후속 작업이 차단될 수 있음 | 팀 리드에게 팀원을 독촉하도록 지시 |
| **종료 지연** | 팀원이 현재 요청이나 도구 호출을 마치고 종료하므로 시간 소요 | 인내심을 갖고 대기 |
| **단일 팀 제한** | 한 세션에서 하나의 팀만 운영 가능 | 현재 팀 해산 후 새 팀 구성 |
| **중첩 팀 불가** | 팀원이 자체적으로 하위 팀이나 팀원을 생성할 수 없음 | 팀 리드만 팀을 관리 |
| **리드 고정** | 팀 리더십을 다른 팀원에게 이전 불가 | 팀 리드를 신중하게 설정 |
| **권한은 생성 시 결정** | 모든 팀원은 팀 리드의 권한 모드로 시작, 이후 개별 변경 가능 | 팀 리드 권한을 사전에 적절히 설정 |
| **Split panes 환경 제한** | VS Code 통합 터미널, Windows Terminal, Ghostty에서 미지원 | in-process 모드 사용 또는 tmux/iTerm2 설치 |
