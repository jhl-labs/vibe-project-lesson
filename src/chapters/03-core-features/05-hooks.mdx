...
Claude Code는 15가지 이벤트 타입을 지원합니다 (Agent Teams의 TeammateIdle, TaskCompleted, 초기화용 Setup 포함). 각 이벤트는 발생 시점, 차단 가능 여부, 매처 대상이 다릅니다.
...
<DataTable
  ...
  data={[
    ...
    { event: 'SessionEnd', timing: '세션 종료', blockable: '아니오', matcher: 'clear, logout, prompt_input_exit, bypass_permissions_disabled, other' },
    { event: 'SubagentStart', timing: '서브에이전트 생성 시', blockable: '아니오', matcher: '에이전트 타입 (Bash, Explore, Plan, 커스텀 등)' },
    { event: 'SubagentStop', timing: '서브에이전트 완료 시', blockable: '예', matcher: '에이전트 타입 (SubagentStart와 동일)' },
    { event: 'Notification', timing: '알림 발생 시', blockable: '아니오', matcher: 'permission_prompt, idle_prompt, auth_success, elicitation_dialog' },
    { event: 'PreCompact', timing: '컨텍스트 압축 전', blockable: '아니오', matcher: 'manual, auto' },
    { event: 'TeammateIdle', timing: '팀원 에이전트 대기 시', blockable: '예', matcher: '(없음)' },
    { event: 'TaskCompleted', timing: '할당된 작업 완료 시', blockable: '예', matcher: '(없음)' },
    { event: 'Setup', timing: 'Hook 초기 설정/설치 시', blockable: '아니오', matcher: '(없음)' },
  ]}
/>
...
| `type` | `"command"` \| `"prompt"` \| `"agent"` \| `"http"` | 핸들러 유형 |
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'http', values: ['외부 HTTP 엔드포인트 호출', '웹훅 전송, 외부 서비스 연동', '30초'] },
  ]}
/>

<Callout type="tip" title="핸들러 선택 기준">
  단순 스크립트 실행은 `command`, 코드 내용을 이해하고 판단해야 하면 `prompt`, 도구를 사용한 복잡한 작업이 필요하면 `agent`, 외부 서비스에 웹훅을 보내거나 API를 호출해야 하면 `http`를 선택하세요.
</Callout>
...
| SessionStart | `compact` | 자동/수동 컴팩션 후 세션 재시작 |
| SessionEnd | `clear` | `/clear`로 세션 종료 |
| SessionEnd | `logout` | 로그아웃으로 종료 |
| SessionEnd | `prompt_input_exit` | 사용자 입력 종료 |
| SessionEnd | `bypass_permissions_disabled` | 권한 우회 모드 비활성화 |
| SessionEnd | `other` | 기타 종료 사유 |
| Notification | `permission_prompt` | 도구 권한 요청 알림 |
| Notification | `idle_prompt` | 60초 이상 입력 대기 알림 |
| Notification | `auth_success` | 인증 성공 알림 |
| Notification | `elicitation_dialog` | 사용자 선택 다이얼로그 알림 |
...
| `$CLAUDE_CODE_REMOTE` | 원격 웹 환경에서 `"true"` 설정 | 모든 이벤트 |
...
// PostToolUse 입력 (tool_response 포함)
{
  "session_id": "abc123",
  "transcript_path": "/path/to/transcript.jsonl",
  "cwd": "/path/to/project",
...
// PostToolUse — 정보 피드백
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "lint 경고 3건 발견: unused-vars(2), no-console(1)"
  }
}`}
  language="json"
...
<MermaidDiagram
  chart={`sequenceDiagram
    ...
    rect rgb(240, 255, 240)
        ...
        CC->>PO: stdin (JSON: tool_name, tool_input, tool_response)
        PO->>CC: 후처리 결과 / 메시지
    end
    ...
    rect rgb(255, 248, 240)
        ...
        CC->>S: stdin (JSON: stop_hook_active)
        alt exit 0
            S->>CC: 작업 완료
        else exit 2
            S->>CC: 차단 + 추가 작업 요청
...
SECRET_PATTERNS=(
  ...
  'password\s*=\s*"[^"]+'     # 하드코딩된 비밀번호(큰따옴표 값)
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "agent",
            "prompt": "모든 유닛 테스트를 실행하고 결과를 확인하세요. 실패한 테스트가 있으면 작업을 차단(block)하고 원인과 수정 방안을 보고하세요.",
            "timeout": 120
          }
        ]
      }
...
# 참고: "npm test"는 스크립트에 인자를 넘기기 위해 "--"가 필요하지만,
#       "npx jest"는 "npx jest --coverage"처럼 "--" 없이 플래그를 직접 받습니다.
INPUT=$(cat)
COMMAND=$(echo "$INPUT" | jq -r '.tool_input.command')

if echo "$COMMAND" | grep -qE '^npm test$'; then
  # 입력 수정하여 coverage 포함
  cat <<EOJSON
{
  "hookSpecificOutput": {
...
`--debug` 플래그를 사용하면 Hook의 실행 상태, stdin/stdout, exit code를 확인할 수 있습니다. 세션 중에는 `Ctrl+O`로 verbose 모드를 토글할 수 있습니다.

<CodeBlock
  code={`# debug 모드로 Claude Code 실행
claude --debug
...
| 핸들러 타입 | 기본 타임아웃 | 타임아웃 시 동작 |
|------------|-------------|----------------|
| command | 600초 (10분) | 프로세스 강제 종료 |
| prompt | 30초 | 응답 무시 |
| agent | 60초 | 에이전트 종료 |
...
# .claude/settings.json의 hooks 설정이 자동으로 로드됨
...
<ChapterNav
  prev={{ title: '에이전트', path: '/docs/part-3--핵심-기능-에이전트' }}
  next={{ title: 'Plugins', path: '/docs/part-3--핵심-기능-plugins' }}
/>
