...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '예시', values: ['/init, /clear, /help, /code-review', '/commit, /pr, /review'] },
  ]}
/>
...
| `claude agents` | Agent view — 모든 Claude Code 세션을 한 화면에서 (실행중/대기/완료 상태) | 여러 세션을 병렬로 운영할 때 |

### 개발 지원

| 커맨드 | 설명 | 주요 사용 시점 |
|--------|------|---------------|
| `/code-review` | 내장 코드 리뷰 — correctness 버그 보고, `--fix`로 수정 적용 (구 `/simplify`에서 개명) | 코드 검토 시 |
| `/usage` | 플랜 한도를 skill/subagent/plugin/MCP 서버 단위로 분해 표시 | 사용량/한도 점검 시 |
| `/recap` | 터미널이 비활성일 때 일어난 일 요약 (세션 복귀 시 recap) | 세션 복귀 시 |
| `/goal` | 완료 조건이 충족될 때까지 턴을 넘어 계속 작업 | 장기 목표 작업 시 |
...
# 모델 선택 화면에서 원하는 모델 지정 (2026.06 기준)
# - claude-opus-4-8: Opus 4.8, 기본/최강 모델 (가장 강력한 추론)
# - claude-sonnet-4-6: Sonnet 4.6, 속도/지능 균형
# - claude-haiku-4-5: Haiku 4.5, 가장 빠른 응답
# - claude-sonnet-4-5-20250929: Sonnet 4.5 (레거시, 사용 가능)
...
| 커맨드 | 설명 | 출력물 | 설정 파일 |
|--------|------|--------|-----------|
| `/review` | 코드 리뷰 (커스텀) | 리뷰 코멘트 및 제안 | `.claude/commands/review.md` |
...
<Callout type="info" title="커스텀 /review vs 내장 /code-review">
  Claude Code의 내장 코드 리뷰 커맨드는 `/code-review`입니다(구 `/simplify`에서 개명, correctness 버그를 보고하고 `--fix`로 수정 적용).
  여기서 정의하는 커스텀 `/review`는 이름이 다르므로 내장 커맨드와 충돌하지 않으며, `.claude/commands/review.md`로
  프로젝트별 리뷰 기준(보안 체크리스트, 컨벤션 검증 등)을 자유롭게 정의할 수 있습니다.
</Callout>
...
<Callout type="info" title="커맨드/스킬 통합">
  최신 Claude Code(2.1.x)에서는 슬래시 커맨드와 스킬이 통합되어, `.claude/commands/`와 `.claude/skills/` 디렉토리 모두에서 마크다운 파일을 정의하면 `/파일명`으로 호출할 수 있습니다.
...
테스트 피라미드 비율(Unit 70%, Integration 20%, E2E 10%)은 권장 가이드라인(프로젝트별 조정)이며, 이를 참고해 AAA 패턴과 FIRST 원칙에 기반한 테스트 코드를 생성합니다.

### 테스트 피라미드

<PlotlyChart
  title="테스트 피라미드 비율 (권장 가이드라인, 프로젝트별 조정)"
  data={[
    {
...
아래 수치는 권장 가이드라인(프로젝트별 조정)입니다. 도메인 핵심 로직일수록 높게, 인프라 연동 코드일수록 현실적인 수준으로 설정하세요.
...
| A09 | 보안 로깅 및 알림 실패 (Security Logging & Alerting Failures) | 감사 로그, 알림 |
| A10 | 예외 처리 미흡 (Mishandling of Exceptional Conditions) | 에러 핸들링, fail-open 방지 |
...
mode: plan                          # 실행 모드 (plan: 코드 수정 없이 분석만)
skills:                             # 이 스킬에서 로드할 하위 스킬 목록
  - api-conventions
---`}
  language="yaml"
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'mode', values: ['string', '없음', 'plan 설정 시 코드 수정 없이 분석/계획만 수행'] },
    { feature: 'skills', values: ['list', '없음', '이 스킬 실행 시 함께 로드할 하위 스킬 목록'] },
  ]}
/>
...
<Callout type="tip" title="Claude Code에서 스킬 실행">
  최신 Claude Code(2.1.x)에서는 `.claude/commands/`와 `.claude/skills/` 디렉토리가 통합되어,
