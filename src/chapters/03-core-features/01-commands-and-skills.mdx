...
# - claude-opus-4-5-20251101: 복잡한 추론, 고품질 코드
...
agent: Explore                      # 서브에이전트 타입 (context: fork 시 사용)
...
<ComparisonTable
  ...
  data={[
    ['name', 'string', '(선택)', '스킬 식별자. 미지정 시 디렉토리명 사용. /name으로 호출'],
    ['description', 'string', '(권장)', 'Claude 자동 매칭에 사용되는 설명. 미지정 시 본문 첫 단락 사용'],
    ['argument-hint', 'string', '없음', '자동완성 시 인자 형식 힌트 (예: [issue-number])'],
    ['context', '"fork" | 없음', '없음', 'fork 시 독립 서브에이전트로 실행'],
    ['agent', 'string', 'general-purpose', 'context: fork 시 사용할 서브에이전트 타입'],
    ['model', 'string', '(상속)', '사용할 모델 (opus/sonnet/haiku)'],
    ['allowed-tools', 'string', '모든 도구', '스킬 활성 시 허용할 도구 제한'],
    ['disable-model-invocation', 'boolean', 'false', 'true면 사용자만 호출 가능'],
    ['user-invocable', 'boolean', 'true', 'false면 Claude만 자동 호출 가능'],
    ['hooks', 'object', '없음', '스킬 라이프사이클에 스코프된 Hook 정의'],
  ]}
/>
...
배포 전 테스트와 린트를 자동 실행하고, 배포 완료 후 로그를 기록합니다.`}
  language="yaml"
