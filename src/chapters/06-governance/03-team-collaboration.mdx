...
[Faros AI의 2025년 연구](https://www.faros.ai/ai-productivity-paradox)(1,255개 팀, 10,000+ 개발자 텔레메트리)에 따르면,
AI 고도 활용 팀에서 작업 완료량은 21% 증가하고 머지 PR은 98% 증가했지만, PR 리뷰 시간은 91% 증가하고 PR 크기는 154% 커졌습니다.
이는 **Amdahl의 법칙** — 시스템은 가장 느린 구간의 속도로만 움직인다 — 이 소프트웨어 개발에도 적용됨을 보여줍니다.
...
MAX_LINES=500
CHANGED_LINES=$(git diff --numstat HEAD~1 | awk '{added+=$1; removed+=$2} END {print added+removed}')
...
**Agent Teams**는 **실험적 기능**으로 제공됩니다. 팀 단위로 활용하면
하나의 작업을 여러 에이전트에게 병렬로 분배하여, 개발-테스트-문서화를 동시에 진행할 수 있습니다.
한 세션이 팀 리드로 작동하여 작업을 조율하고, 각 팀원은 독립된 컨텍스트 윈도우에서 작업하며
서로 직접 소통할 수 있습니다(서브에이전트와 달리 메인 에이전트를 거치지 않아도 됨).
현행 기본 모델인 Opus 4.8 환경에서도 동일하게 동작하며, 특정 모델 버전에 종속되지 않습니다.

<Callout type="warning" title="실험적 기능">
  Agent Teams는 기본적으로 비활성화되어 있습니다. `settings.json`이나 환경 변수에
  `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`를 추가하여 활성화하세요.
  조율 오버헤드로 인해 단일 세션보다 토큰 사용량이 크게 증가하며,
  세션 재개, 작업 조율, 종료 동작에 제한이 있습니다.
</Callout>

### Dynamic workflows: 대규모 백그라운드 오케스트레이션

2026년 5월 도입된 **Dynamic workflows**는 Agent Teams를 한층 확장합니다.
Claude이 직접 스크립트를 작성해 **수십~수백 개의 서브에이전트를 백그라운드에서 오케스트레이션**하므로,
팀 리드가 모든 분배를 수동으로 지시하지 않아도 대규모 작업을 동적으로 쪼개어 병렬 처리할 수 있습니다.
대규모 리팩토링, 광범위한 테스트 생성, 멀티 모듈 마이그레이션처럼 작업 단위가 많은 경우에 특히 유용합니다.

<Callout type="info" title="Agent view로 진행 상황 한눈에 보기">
  `claude agents` 명령으로 제공되는 **Agent view**를 사용하면 실행 중인 모든 Claude Code 세션을
  한 화면에서 확인할 수 있습니다. 각 에이전트의 실행 중/대기/완료 상태가 표시되므로,
  여러 에이전트가 백그라운드에서 동시에 작업하는 Agent Teams 및 dynamic workflows 환경에서
  진행 상황을 추적하고 병목을 파악하기에 적합합니다.
  (출처: [Claude Code What"s New](https://code.claude.com/docs/en/whats-new))
</Callout>
...
2026년 1월 12일 리서치 프리뷰로 출시된 **Claude Cowork**는 PM, 디자이너, QA 등 비개발자도 AI 에이전트를 활용할 수 있게 합니다.
Cowork는 Claude Code와 동일한 에이전트 아키텍처를 기반으로 하되, 터미널 대신 **Claude 데스크톱 앱** 안에서 실행됩니다.
사용자가 원하는 폴더에 대한 접근 권한을 부여하면, 복잡한 멀티스텝 작업을 자율적으로 수행합니다.
처음에는 Max 전용이었으나 현재는 Pro, Team, Enterprise 플랜에서도 사용 가능하며, Windows·macOS 데스크톱 앱에서 사용할 수 있습니다.
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'QA', values: ['테스트 시나리오 자동 생성, 리그레션 테스트 스크립트 작성', '수동 테스트 시간 단축'] },
...
<ComparisonTable
  ...
  headers={['기업', '규모', '성과', '출처']}
  rows={[
    { feature: 'Ramp (핀테크)', values: ['엔지니어링 팀 전체 + 제품/디자인/데이터 팀', '월 100만줄+ AI 코드 처리, 인시던트 조사 시간 80% 단축, 비기술 직원도 Snowflake 데이터 직접 조회', 'Anthropic 고객 사례'] },
    { feature: 'Bridgewater (헤지펀드)', values: ['AIA Labs 투자 분석팀', '당시 Claude Opus 4 + Amazon Bedrock로 투자 분석 어시스턴트 구축. 1년차 애널리스트 수준 정밀도, 인사이트 도출 시간 50-70% 단축(사례 발표 시점 기준)', 'Amazon Bedrock 사례'] },
    { feature: 'Anthropic 내부', values: ['엔지니어링 조직 전체 (132명 조사)', '엔지니어당 일일 머지 PR 수 67% 증가, AI 사용 비율 28%→59%, 엔지니어당 코드 생산량 200% 증가 (1년간)', '공식 블로그 (2025.08)'] },
  ]}
/>
...
<ChapterNav
  prev={{ title: '법적 윤리적 고려사항', path: '/docs/part-6--거버넌스-법적-윤리적-고려사항' }}
  next={{ title: '효과 측정', path: '/docs/part-6--거버넌스-효과-측정' }}
/>
