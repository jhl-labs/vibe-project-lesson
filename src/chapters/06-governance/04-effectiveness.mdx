...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '리뷰 시간 증가', values: ['AI 보조 리뷰 도입', 'AI가 1차 리뷰(린트, 패턴 검사) → 인간이 2차 리뷰(설계, 비즈니스 로직). Claude Code Review(2026.03) 등 멀티 에이전트 리뷰 도구 활용'] },
...
<Callout type="info" title="에이전트형 도구의 차별화">
  ...
  2026년 3월에는 **Code Review** 기능이 출시되어 멀티 에이전트 시스템이 모든 PR을 분석합니다.
...
<Callout type="info" title="Claude Code 구독 vs API 비용">
  ...
  Max 구독($100/월, Opus 4.6 포함 5x 사용량)은 API 직접 사용 대비 상당한 비용 절감 효과가 있습니다.
  API 가격 기준: Opus 4.6은 입력 $5/MTok, 출력 $25/MTok; Sonnet 4.6은 입력 $3/MTok, 출력 $15/MTok.
