...
<Callout type="info" title="출처: Techreviewer 2025 AI Development Workflows 조사">
  아래 수치는 [Techreviewer의 2025년 AI 개발 워크플로우 조사](https://techreviewer.co/blog/how-ai-reshaping-development-workflows-in-2025)에서 산출된 효율성 점수(0~1.0)입니다.
...
<ComparisonTable
  ...
  headers={['지표', '조사 결과', '출처']}
  rows={[
    { feature: '생산성 향상', values: ['AI 고도 활용 팀은 작업 완료량 21% 증가, 머지 PR 98% 증가. 그러나 PR 리뷰 시간 91% 증가, PR 크기 154% 증가', 'Faros AI 2025 (1,255개 팀, 10,000+ 개발자 텔레메트리)'] },
    { feature: '팀 생산성 향상', values: ['62%의 응답자가 AI 도구로 25% 이상 개발 속도 향상을 체감. 90%의 팀이 AI 코딩 도구 사용 (전년 61%에서 증가)', 'Jellyfish 2025 State of Engineering Management (640+ CTO/매니저/엔지니어)'] },
    { feature: '코드 리뷰 품질', values: ['AI 코드 리뷰 활용 팀의 81%가 품질 개선을 경험 (미사용 팀 55%). 그러나 76%가 AI 생성 코드를 완전히 신뢰하지 않음', 'Qodo 2025 State of AI Code Quality (609명 개발자)'] },
    { feature: '테스트 신뢰도', values: ['AI 테스트 작성 활용 시 신뢰도 61% (미사용 시 27%). 60%가 AI 도구가 핵심 컨텍스트를 누락한다고 응답', 'Qodo 2025 State of AI Code Quality'] },
    { feature: '전체 코드 중 AI 비율', values: ['2024년 기준 전체 코드의 41%가 AI 도구로 작성 또는 보조 (업계 종합 추정치)', '복수 조사 종합; GitClear는 코드 품질 지표(중복·변동률) 분석'] },
  ]}
/>

*출처: [Faros AI](https://www.faros.ai/ai-productivity-paradox), [Jellyfish](https://jellyfish.co/resources/2025-state-of-engineering-management-report/), [Qodo](https://www.qodo.ai/reports/state-of-ai-code-quality/).*
*Anthropic 내부 데이터: [How AI Is Transforming Work at Anthropic](https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic) — 엔지니어당 머지 PR 67% 증가, 코드 생산량 200% 증가(1년간), AI 사용 비율 28%→59%*
...
<Callout type="warning" title="AI 생산성 역설 (Productivity Paradox)">
  ...
  핵심 원인은 **Amdahl의 법칙**: AI가 코딩 단계를 가속화하더라도, 리뷰·테스트·릴리스 파이프라인이
  새로운 속도를 따라잡지 못하면 전체 성과가 정체됩니다. 라이프사이클 전체의 현대화 없이는
  AI의 이점이 빠르게 상쇄됩니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '리뷰 시간 증가', values: ['AI 보조 리뷰 도입', 'AI가 1차 리뷰(린트, 패턴 검사) → 인간이 2차 리뷰(설계, 비즈니스 로직). /code-review(구 /simplify, --fix로 수정 적용)나 claude ultrareview(클라우드 버그 헌팅 fleet) 등 멀티 에이전트 리뷰 도구 활용'] },
...
<Callout type="info" title="에이전트형 도구의 차별화">
  ...
  이후 멀티 에이전트 리뷰가 발전해, 현재는 **/code-review**(구 /simplify, --fix로 수정 적용)와
  **claude ultrareview**(클라우드에서 버그 헌팅 에이전트 fleet 실행)로 모든 PR을 분석할 수 있습니다.
  Anthropic 내부에서는 Code Review 도입 후 실질적 리뷰 코멘트가 달린 PR 비율이 16%에서 54%로 증가했습니다
  (출처: [Claude Code Docs — Code Review](https://code.claude.com/docs/en/code-review)).
</Callout>
...
<ComparisonTable
  ...
  rows={[
    { feature: '라이선스 (Max 구독)', values: ['$100/인/월 x 10명 x 12개월', '약 1,440만원', '환율 1,200원 기준 (변동 가능)'] },
...
# 첫 요청: $0.3125 (50K x $5/M x 1.25 + α)
r1 = client.messages.create(model="claude-opus-4-8", system=system,
    messages=[{"role": "user", "content": "인증 모듈의 버그를 찾아줘"}])

# 후속 요청: $0.025 (50K x $5/M x 0.10 + α) — 90% 절감!
r2 = client.messages.create(model="claude-opus-4-8", system=system,
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'Anthropic 마케팅팀', values: ['수시간/배치', '수분/배치 (Figma 연동)', '대폭 단축'] },
    { feature: 'Anthropic 보안팀', values: ['수동 분석', '스택 트레이스 자동 분석 + 런북 생성', '대폭 단축'] },
  ]}
/>

<Callout type="info" title="Claude Code 구독 vs API 비용">
  Claude Code의 평균 일일 비용은 **$6**이며, 90%의 개발자가 **$12/일** 이하를 사용합니다 (Anthropic 공식 문서).
  Max 구독($100/월, Opus 4.8 포함 5x 사용량)은 API 직접 사용 대비 상당한 비용 절감 효과가 있습니다.
  API 가격 기준: Opus 4.8은 입력 $5/MTok, 출력 $25/MTok; Sonnet 4.6은 입력 $3/MTok, 출력 $15/MTok.
  Team 플랜의 프리미엄 좌석($150/월)은 Claude Code 개발 환경을 포함합니다.
</Callout>

---

<ChapterNav
  prev={{ title: '팀 협업', path: '/docs/part-6--거버넌스-팀-협업' }}
/>
