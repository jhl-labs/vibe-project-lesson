...
OWASP는 2025년 **[Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/)**에서 프롬프트 인젝션을 1위(LLM01),
2026년 **[Top 10 for Agentic Applications](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/)**에서 에이전트 목표 탈취(ASI01 Agent Goal Hijack)를 1위로 선정했습니다.
Agentic Top 10은 100명 이상의 보안 전문가가 참여하여 2025년 12월에 공개되었으며,
에이전트를 **목표·도구·메모리·프로토콜을 가진 주체(principal)**로 취급하고 각각을 공격 표면으로 식별합니다.
...
2025년 8월 공개된 GitHub Copilot의 [CVE-2025-53773](https://nvd.nist.gov/vuln/detail/CVE-2025-53773)은 소스 코드 파일이나 GitHub 이슈에 삽입된
악성 프롬프트를 통해 원격 코드 실행이 가능했던 실제 사례입니다(CVSS 7.8, Microsoft 2025년 8월 Patch Tuesday에서 수정).
공격자는 `.vscode/settings.json`에 `"chat.tools.autoApprove": true`를 주입하여 Copilot을 "YOLO 모드"로 전환시킨 후
사용자 승인 없이 임의 명령을 실행할 수 있었습니다.
...
<Callout type="tip" title="개발자 실천 사항">
  ...
  2026년부터 Anthropic은 **샌드박싱을 모든 환경의 기본 설정으로 권장**합니다.
  샌드박스는 Linux bubblewrap / macOS seatbelt을 사용하여 OS 수준에서
  파일 시스템과 네트워크를 격리하며, 내부 테스트에서 권한 프롬프트를 **84% 감소**시키면서
  **15ms 미만의 지연**만 추가합니다.
</Callout>
...
check_npm_package() {
  ...
  #    npm registry 메타데이터에는 다운로드 수가 없으므로 다운로드 통계 API를 사용
  local downloads
...
OWASP가 제시하는 핵심 방어 원칙은 **최소 에이전시 원칙(Least Agency)** — AI 에이전트에게
의도된 작업 수행에 필요한 최소한의 자율성, 도구 접근, 자격 증명 범위만 부여하는 것입니다.
...
# 시크릿 패턴 정의
PATTERNS=(
  '[A-Za-z0-9_]*(KEY|SECRET|TOKEN|PASSWORD|CREDENTIAL)[A-Za-z0-9_]*\s*=\s*"[^"]{8,}'
  'sk-[a-zA-Z0-9]{20,}'
  'ghp_[a-zA-Z0-9]{36}'
  'AKIA[0-9A-Z]{16}'
...
for pattern in "\${BLOCKED_PATTERNS[@]}"; do
  if echo "$COMMAND" | grep -qP "$pattern"; then
    echo "{\\"hookSpecificOutput\\": {\\"hookEventName\\": \\"PreToolUse\\", \\"permissionDecision\\": \\"deny\\", \\"permissionDecisionReason\\": \\"보안 정책 위반: '$pattern' 패턴이 감지되었습니다\\"}}"
    exit 0
  fi
done
...
# 1. 하드코딩된 시크릿 탐지
if grep -qiP '(api_key|secret|password|token)\s*=\s*"[^"]{8,}' "$FILE_PATH" 2>/dev/null; then
  ISSUES+=("하드코딩된 시크릿 감지")
fi
...
<ComparisonTable
  title="OWASP Top 10 (2025) 웹 애플리케이션 보안 점검"
  headers={['순위', '취약점', 'AI 코드 위험도', '대응 방법']}
  rows={[
    { feature: 'A01', values: ['접근 제어 취약점 (Broken Access Control)', '높음', '모든 엔드포인트 인가 확인'] },
    { feature: 'A02', values: ['보안 설정 오류 (Security Misconfiguration)', '높음', '기본 설정 변경, 최소 권한'] },
    { feature: 'A03', values: ['소프트웨어 공급망 실패 (Supply Chain Failures)', '높음', '의존성 스캔 자동화, lock 파일 커밋'] },
    { feature: 'A04', values: ['암호화 실패 (Cryptographic Failures)', '중간', '검증된 암호화 라이브러리 사용'] },
    { feature: 'A05', values: ['인젝션 (Injection)', '높음', '파라미터화 쿼리, ORM 사용'] },
    { feature: 'A06', values: ['불안전한 설계 (Insecure Design)', '중간', '위협 모델링 수행'] },
    { feature: 'A07', values: ['인증 실패 (Identification and Authentication Failures)', '중간', '검증된 인증 라이브러리 사용'] },
    { feature: 'A08', values: ['데이터 무결성 실패 (Data Integrity Failures)', '중간', '서명 검증, 공급망 보안'] },
    { feature: 'A09', values: ['보안 로깅 및 모니터링 실패 (Security Logging Failures)', '중간', '구조화된 로깅과 알림 구현'] },
    { feature: 'A10', values: ['예외 처리 미흡 (Mishandling of Exceptional Conditions)', '중간', '체계적 에러 핸들링 구현'] },
  ]}
/>
...
<Callout type="tip" title="Claude Code의 보안 지원">
  ...
  또한 **Auto mode**(2026 research preview)는 분류기가 권한 프롬프트를 처리하여
  안전한 작업은 자동 실행하고 위험한 작업은 차단합니다. hard deny 규칙을 함께 지정하면
  "전부 자동 승인"과 "전부 수동 확인"의 중간 지점에서 안전하게 운용할 수 있습니다.
</Callout>
...
2026년 2월 5일, Anthropic의 Frontier Red Team은 Claude Opus 4.6이 **프로덕션 오픈소스 소프트웨어에서 500개 이상의 고심각도 제로데이 취약점을 발견**했다고 발표했습니다.
...
<ComparisonTable
  ...
  headers={['항목', '내용']}
  rows={[
    { feature: '사용 도구', values: ['Opus 4.6 + Python, 디버거, 퍼저 등 기본 분석 도구 (전문 프롬프팅이나 커스텀 스캐폴딩 없음)'] },
    { feature: '대상', values: ['수년간 퍼저(fuzzer)가 실행된 가장 잘 테스트된 프로덕션 오픈소스 코드베이스'] },
    { feature: '방법', values: ['랜덤 입력 생성이 아닌 코드 읽기·추론 — 데이터 흐름 추적, 커밋 히스토리 분석, 부분 수정 버그의 변종 탐지'] },
    { feature: '결과', values: ['500+ 고심각도 제로데이 취약점 (수십 년간 전문가 리뷰와 퍼저를 통과한 버그 포함)'] },
    { feature: '검증', values: ['Anthropic Frontier Red Team 및 외부 보안 연구원이 모든 취약점 확인'] },
  ]}
/>
...
<ComparisonTable
  ...
  headers={['인사이트', '실천 방안']}
  rows={[
    { feature: 'AI는 인간이 놓친 취약점을 찾을 수 있다', values: ['security-review 커맨드, claude ultrareview(/ultrareview), security-guidance 플러그인(2026.05) 또는 보안 서브에이전트를 정기적으로 실행'] },
    { feature: '전통적 보안 도구로는 부족하다', values: ['퍼저 + AI 코드 리뷰를 병행하는 하이브리드 접근'] },
    { feature: 'AI 보안 분석은 비용 효율적이다', values: ['Anthropic Batch API로 전체 코드베이스 보안 스캔을 비동기 배치로 처리하면 표준 요청 대비 50% 할인이 적용됩니다(출처: Anthropic Batch API 문서)'] },
    { feature: '새로운 위협도 있다', values: ['AI 생성 코드의 취약점을 AI로 검증하는 이중 검증 패턴 도입'] },
  ]}
/>

<Callout type="info" title="Claude Code의 실제 보안 리뷰 도구">
  Claude Code는 다음과 같은 보안 리뷰 수단을 제공합니다.
  - **security-review 커맨드**: 변경된 코드를 보안 관점에서 점검합니다.
  - **claude ultrareview / /ultrareview**(2026): 클라우드에서 버그 헌팅 에이전트 fleet을 실행하고 결과를 CLI/Desktop으로 자동 전달합니다. CI/스크립트에서도 사용할 수 있습니다.
  - **security-guidance 플러그인**(2026.05): 작업 중 Claude의 변경사항을 취약점 관점에서 리뷰합니다.
  - **보안 서브에이전트**: `.claude/agents/`에 보안 전용 에이전트를 정의해 정기 스캔에 활용할 수 있습니다.
</Callout>
...
**출처:** [Anthropic Frontier Red Team Blog](https://red.anthropic.com/2026/zero-days/), [Axios](https://www.axios.com/2026/02/05/anthropic-claude-opus-46-software-hunting), [VentureBeat](https://venturebeat.com/security/anthropic-claude-code-security-reasoning-vulnerability-hunting), [The Hacker News](https://thehackernews.com/2026/02/claude-opus-46-finds-500-high-severity.html)
