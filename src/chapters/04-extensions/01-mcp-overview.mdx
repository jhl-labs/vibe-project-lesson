...
<Timeline
  ...
  events={[
    ...
    {
      date: '2025-06-18',
      title: 'Elicitation 기능 추가 (스펙 2025-06-18)',
      description: 'MCP 서버가 실행 중 사용자에게 추가 정보를 요청할 수 있는 elicitation 기능 도입. Form 모드와 URL 모드 두 가지를 지원.',
    },
    {
      date: '2025-09-08',
      title: 'MCP Registry 프리뷰 출시',
      description: '공식 MCP 서버 레지스트리(registry.modelcontextprotocol.io)가 프리뷰로 출시. 검증된 서버를 쉽게 발견하고 설치 가능.',
    },
    {
      date: '2025-11-25',
      title: '스펙 버전 2025-11-25 릴리스 (MCP 1주년)',
      description: '최신 스펙 버전. 비동기 Tasks(call-now, fetch-later), 향상된 OAuth(CIMD, Step-Up Authorization), Extensions 프레임워크, URL 모드 Elicitation, 엔터프라이즈 관리형 인가 포함.',
      highlight: true,
    },
    {
      date: '2025-12-09',
      title: 'AAIF 기부 (Linux Foundation)',
      description: 'Anthropic이 MCP를 Agentic AI Foundation에 기부. AWS, OpenAI, Google, Microsoft, Block, Bloomberg, Cloudflare 등 Platinum 멤버로 참여.',
      highlight: true,
    },
    {
      date: '2026-01-14',
      title: 'Tool Search Tool 공식 출시',
      description: 'MCP 도구를 동적으로 검색하는 Tool Search가 기본 활성화. 토큰 85%+ 절감, 정확도 향상.',
    },
  ]}
/>
...
| **time** | 시간대 변환 | 없음 |
| **everything** | 테스트/레퍼런스 구현 | 없음 |

<Callout type="info" title="Archived 서버와 대안">
  postgres, sqlite, puppeteer, slack, **github**, gitlab, sentry, redis, brave-search, google-drive, google-maps 등 14개 레퍼런스 서버는 2025년 5월 `modelcontextprotocol/servers-archived`로 이동되어 더 이상 유지보수되지 않습니다.
  DB 연동은 **DBHub**(`@bytebase/dbhub`), GitHub는 **GitHub 공식 MCP 서버**(`github/github-mcp-server`) 또는 **HTTP 방식**(`https://api.githubcopilot.com/mcp/`), Slack은 커뮤니티 서버 사용을 권장합니다.
</Callout>
...
# SQLite (DBHub 사용 — server-sqlite는 archived)
claude mcp add --transport stdio sqlite -- \\
  npx -y @bytebase/dbhub \\
  --dsn "sqlite:///./data/app.sqlite"
...
## Tool Search Tool -- 토큰 85% 절감 (2026년 1월~)

MCP 서버가 50개 이상의 도구를 노출할 수 있고, 사용자가 7개 이상의 서버를 동시에 실행하면 67,000+ 토큰이 도구 정의에만 소비됩니다. **Tool Search Tool**은 이 문제를 해결합니다. 2026년 1월 14일 Anthropic이 공식 발표했으며, 현재 모든 사용자에게 기본 활성화되어 있습니다.
...
→ 대규모 토큰 소비, 정확도 49% (Opus 4)

[Tool Search] 필요한 도구만 동적 검색
→ 토큰 85% 이상 감소, 정확도 74% (Opus 4)
→ 정확도 +25%p 향상`}
  language="text"
...
<ComparisonTable
  title="Tool Search 사용 전후 비교 (Anthropic 내부 테스트)"
  headers={['모델', 'Tool Search 미사용', 'Tool Search 사용', '정확도 향상']}
  rows={[
    { feature: 'Opus 4', values: ['49%', '74%', '+25%p'] },
    { feature: 'Opus 4.5', values: ['79.5%', '88.1%', '+8.6%p'] },
  ]}
/>
...
Anthropic은 MCP 서버의 코드 실행을 안전하게 격리하기 위한 **sandbox-runtime**을 Apache 2.0 라이선스로 오픈소스 공개했습니다. 내부 사용 결과 권한 프롬프트를 **84% 감소**시키는 효과가 있었습니다.

<CodeBlock
  code={`# 네이티브 OS 샌드박싱 사용
# macOS: seatbelt, Linux: bubblewrap
# 컨테이너 없이 파일 시스템 및 네트워크 접근 제어
# Claude Code에서 /sandbox 명령으로 활성화
...
<ComparisonTable
  ...
  rows={[
    { feature: '관리 주체', values: ['Anthropic 단독', 'Agentic AI Foundation (Linux Foundation)'] },
    { feature: '거버넌스', values: ['Anthropic 결정', '오픈 거버넌스 (다수 기업 참여)'] },
    { feature: '참여 기업', values: ['Anthropic', 'Anthropic, OpenAI, Google, Microsoft, AWS, Block, Bloomberg, Cloudflare 등'] },
    { feature: '라이선스', values: ['MIT', 'MIT (변경 없음)'] },
    { feature: '스펙 발전', values: ['Anthropic 주도', '커뮤니티 RFC 프로세스'] },
  ]}
/>
...
<Callout type="tip" title="업계 표준으로 자리잡는 MCP">
  MCP가 Linux Foundation으로 이관됨에 따라, AI 에이전트의 도구 통합이 업계 표준으로 자리잡고 있습니다. 수천 개 이상의 MCP 서버가 공개되어 있으며, OpenAI, Google도 MCP 지원을 발표했습니다. Block의 goose와 OpenAI의 AGENTS.md(다수의 오픈소스 프로젝트가 채택)도 함께 AAIF의 창립 프로젝트로 기부되어 에이전트 AI 생태계가 빠르게 발전하고 있습니다.
</Callout>
