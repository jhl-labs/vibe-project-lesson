...
**Claude Agent SDK**(구 Claude Code SDK)는 Python과 TypeScript 패키지로 제공되며, Claude Code를 프로그래밍 방식으로 완전히 제어할 수 있습니다. 2025년 말 범용 에이전트 런타임으로 리브랜딩되었으며, CLI `-p` 플래그보다 훨씬 정교한 제어가 가능합니다. 2026년 3월 현재 TypeScript는 npm v0.2.x, Python은 PyPI v0.1.x 버전입니다.
