...
<Meta title="Part 4: 확장과 자동화/Agent SDK와 자동화" />
...
<Callout type="info" title="Claude Code SDK → Claude Agent SDK로 이름 변경">
  이전에는 "Claude Code SDK"라는 이름이었지만, 2025년 말 코딩 전용이 아닌 범용 에이전트 런타임으로 발전함에 따라 **Claude Agent SDK**로 이름이 변경되었습니다.
  TypeScript 패키지는 `@anthropic-ai/claude-code`에서 `@anthropic-ai/claude-agent-sdk`로, Python 패키지는 `claude-code-sdk`에서 `claude-agent-sdk`로 변경되었습니다.
...
# 결과 예시 (structured_output 필드에 파싱된 객체):
# {
#   "result": "분석 텍스트...",
#   "structured_output": {"severity":"warning","issues":[{"line":15,...}],"summary":"...","score":72},
#   "session_id": "...",
...
<Callout type="tip" title="JSON Schema 활용 팁">
  ...
  파싱 방식은 한 가지로 통일하세요. `--json-schema`를 지정하면 파싱된 객체가 최상위 `structured_output` 필드에 담기므로, jq에서는 `jq -r ".structured_output.score"`처럼 직접 접근하면 됩니다(`.result | fromjson` 같은 이중 파싱 불필요). 비용은 `total_cost_usd` 필드로 통일되어 있습니다. Agent SDK에서도 동일하게 `message.structured_output`과 `message.total_cost_usd`를 사용합니다.
</Callout>
...
**Claude Agent SDK**(구 Claude Code SDK)는 Python과 TypeScript 패키지로 제공되며, Claude Code를 프로그래밍 방식으로 완전히 제어할 수 있습니다. 2025년 말 범용 에이전트 런타임으로 리브랜딩되었으며, CLI `-p` 플래그보다 훨씬 정교한 제어가 가능합니다. 패키지 버전은 빠르게 갱신되므로, npm(`@anthropic-ai/claude-agent-sdk`)과 PyPI(`claude-agent-sdk`)의 최신 릴리스를 설치 시점에 확인하세요.
...
<CodeBlock
  code={`// Claude Agent SDK 설치 (구 Claude Code SDK)
...
async function toolApprovalExample() {
  for await (const message of query({
      ...
      canUseTool: async (toolName, input, { signal }) => {
        ...
        if (toolName === "Bash") {
          ...
          if (command.includes("rm") && !command.includes("node_modules")) {
            return {
              behavior: "deny",
              message: "node_modules 외 파일 삭제는 금지됩니다",
            };
          }
        }
    ...
    if (message.type === "result") {
      console.log("작업 완료:", message.subtype);
    }
  }
}`}
  language="typescript"
...
<CodeBlock
  code={`# Claude Agent SDK 설치 (구 Claude Code SDK, Python 3.10+ 필요)
...
async def basic_usage():
    async for message in query(
        ...
        if isinstance(message, ResultMessage):
            print(f"비용: {message.total_cost_usd} USD")
...
async def streaming_usage():
    ...
    async for message in query(
        ...
        if isinstance(message, AssistantMessage):
            ...
            # 최종 결과
            print(f"[완료] 비용: {message.total_cost_usd} USD")
...
<ComparisonTable
  ...
  rows={[
    { feature: '설치', values: ['Claude Code CLI만 필요', 'npm: @anthropic-ai/claude-agent-sdk / pip: claude-agent-sdk'] },
    { feature: '실행 방식', values: ['셸 명령어', '프로그래밍 코드'] },
    { feature: '출력 형식', values: ['text, json, stream-json', '네이티브 객체'] },
    { feature: '도구 승인', values: ['--allowedTools 목록', 'canUseTool 콜백 (동적 제어)'] },
    ...
    { feature: '구조화 출력', values: ['--json-schema (문자열)', 'outputFormat (객체)'] },
    ...
    steps:
      ...
      - name: Analyze PR Changes
        ...
        run: |
          ...
          # 결과 파싱 (--json-schema 사용 시 structured_output에 파싱된 객체가 담김)
          SCORE=$(echo "$REVIEW" | jq -r '.structured_output.score')
          CRITICALS=$(echo "$REVIEW" | jq -r '.structured_output.critical_issues | length')
          ...
          if [ "$CRITICALS" -gt 0 ]; then
            ...
            echo "$REVIEW" | jq -r '.structured_output.critical_issues[]'
            exit 1
          fi
...
# 결과 출력 (--json-schema 사용 시 structured_output에 파싱된 객체가 담김)
PARSED=$(echo "$RESULT" | jq -r '.structured_output')
...
TITLE=$(echo "$COMMIT_MSG" | jq -r '.structured_output.title')
BODY=$(echo "$COMMIT_MSG" | jq -r '.structured_output.body')
...
4. 즉시 조치 사항 제안" \\
    ...
    PARSED=$(echo "$ANALYSIS" | jq -r '.structured_output')
...
async function reviewPR(owner: string, repo: string, prNumber: number) {
  ...
  // Claude로 리뷰
  let review: ReviewResult = { score: 0, issues: [], summary: "", approved: false };
  let totalCost = 0;
...
보안, 성능, 코드 품질을 중점 검토하세요.\`,
    ...
    if (message.type === "result" && message.subtype === "success") {
      if (message.structured_output) {
        review = message.structured_output as ReviewResult;
      }
      totalCost = message.total_cost_usd;
    }
  }
...
플레이스홀더({name}, {count} 등)는 그대로 유지하세요." \\
  ...
  # 번역 결과를 파일로 저장 (structured_output에 파싱된 객체가 담김)
  echo "$RESULT" | jq -r '.structured_output' > "$TARGET_FILE"
  echo "완료: $TARGET_FILE"
done
...
async function generateHealthReport(): Promise<HealthReport> {
  let report: HealthReport = {} as HealthReport;
  let totalCost = 0;
...
src/ 디렉토리를 분석하세요.\`,
    options: {
      ...
      outputFormat: { type: "json_schema", schema: healthSchema },
    },
  })) {
    if (message.type === "result" && message.subtype === "success") {
      if (message.structured_output) {
        report = message.structured_output as HealthReport;
      }
      totalCost = message.total_cost_usd;
    }
  }
...
async function multiAgentPipeline(task: string) {
  ...
  // 헬퍼: query 결과에서 구조화 출력과 비용 추출
  async function collectResult(iter: AsyncIterable<any>) {
  ...
  const { structured: analysis, cost: analysisCost } = await collectResult(
    query({
...
를 JSON으로 반환하세요.\`,
  ...
  const { cost: codingCost } = await collectResult(
    query({
...
각 파일을 계획대로 수정하고, 테스트도 함께 작성하세요.\`,
  ...
  const { structured: review, cost: reviewCost } = await collectResult(
    query({
...
4. 최종 승인 여부를 판단해주세요.\`,
  ...
  // 총 비용 계산
  const totalCost = analysisCost + codingCost + reviewCost;
      ...
      canUseTool: async (toolName, input, { signal }) => {
        ...
        if (toolName === "Edit") {
          ...
          if (editCount > MAX_EDITS) {
            return {
              behavior: "deny",
              message: \`최대 편집 횟수(\${MAX_EDITS})에 도달했습니다\`,
            };
          }
          ...
          if (protectedPaths.some(p => filePath.endsWith(p))) {
            return {
              behavior: "deny",
              message: \`설정 파일(\${filePath})은 수정할 수 없습니다\`,
            };
          }
...
COMPLEXITY=$(echo "$ESTIMATE" | jq -r '.structured_output.complexity')
EST_TURNS=$(echo "$ESTIMATE" | jq -r '.structured_output.estimated_turns')
ESTIMATE_COST=$(echo "$ESTIMATE" | jq -r '.total_cost_usd')
...
ACTUAL_COST=$(echo "$RESULT" | jq -r '.total_cost_usd')
echo ""
echo "=== 실행 완료 ==="
...
# 해결:
claude -p "작업해줘" --max-turns 10 --model claude-sonnet-4-6
...
직접 스크립트를 작성하는 Headless/SDK 방식 외에도, Claude Code는 2026년에 들어 별도 인프라 없이 자동화를 구성할 수 있는 내장 기능을 확장했습니다. 반복 작업이나 PR 게이트를 빠르게 구성할 때 먼저 검토할 만한 옵션입니다.

<ComparisonTable
  title="2026 내장 자동화 기능"
  headers={['기능', '설명', '용도']}
  rows={[
    { feature: 'Routines (웹)', values: ['스케줄/GitHub 이벤트/API 호출로 템플릿 클라우드 에이전트를 실행', '정기 리포트, 이벤트 기반 트리거 — 직접 크론/큐 구축 대체'] },
    { feature: '/code-review --fix', values: ['correctness 버그를 보고하고 --fix로 수정까지 적용', 'PR 게이트, 로컬 리뷰 (이전 /simplify에서 이름 변경)'] },
    { feature: 'claude ultrareview', values: ['클라우드에서 버그 헌팅 에이전트 fleet을 실행하고 결과를 CLI/Desktop으로 자동 전달', '대규모 리뷰, CI/스크립트에서도 호출 가능'] },
    { feature: 'security-guidance 플러그인', values: ['작업 중 변경사항을 취약점 관점에서 자동 리뷰', '보안 게이트'] },
  ]}
/>

<CodeBlock
  code={`# 로컬/CI에서 correctness 버그 리뷰 후 수정까지 적용
claude /code-review --fix

# 클라우드 fleet으로 심층 버그 헌팅 — 결과가 CLI/Desktop으로 자동 전달
claude ultrareview

# CI 스텝에서 비대화형으로 호출하는 예시
claude -p "/code-review" --allowedTools "Read,Glob,Grep" --max-turns 5`}
  language="bash"
  filename="2026 클라우드 자동화 명령어"
/>

<Callout type="tip" title="언제 무엇을 쓸까">
  단순한 정기 작업이나 이벤트 트리거는 **Routines**로 충분합니다.
  PR마다 자동 리뷰가 필요하면 **/code-review --fix**를, 릴리스 전 대규모 버그 헌팅이 필요하면 **claude ultrareview**를 고려하세요.
  세밀한 도구 제어, 멀티 에이전트 오케스트레이션, 커스텀 출력 파싱이 필요할 때 비로소 Headless/Agent SDK 직접 구현으로 내려가는 것이 효율적입니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '--model', values: ['사용할 모델 지정', '--model claude-sonnet-4-6'] },
...
<Callout type="tip" title="핵심 정리">
  ...
  **Claude Agent SDK**(구 Claude Code SDK, `@anthropic-ai/claude-agent-sdk` / `claude-agent-sdk`)는 Python/TypeScript에서 더 정교한 제어 (도구 승인 콜백, 스트리밍, 멀티 에이전트)가 필요할 때 사용합니다.
  두 방식 모두 JSON Schema 기반 구조화 출력을 지원하여, 자동화 파이프라인에서 결과를 프로그래밍적으로 처리할 수 있습니다.
</Callout>

<ChapterNav
  prev={{ title: 'CI/CD 통합', path: '/docs/part-4--확장과-자동화-ci-cd-통합' }}
