...
- **모델 선택**: 간단한 작업에는 Haiku 4.5(`claude-haiku-4-5`)나 Sonnet 4.6(`claude-sonnet-4-6`)을, 복잡한 작업에만 Opus 4.8(`claude-opus-4-8`)을 사용
- **노력(effort) 제어**: Opus 4.8은 effort 파라미터가 기본 `high`입니다. CI는 비대화형이므로 슬래시 커맨드(`/effort`)가 아니라 `claude_args`로 전달합니다 — 간단한 작업엔 `claude_args: "--effort low"`로 토큰을, 고난도 작업엔 `claude_args: "--effort xhigh"`로 품질을 우선해 비용을 조절하세요
...
- **도구 제한**: `claude_args: "--allowedTools ..."`로 허용할 도구를 명시적으로 지정(v1.0에서 개별 옵션이 `claude_args`로 통합됨)
