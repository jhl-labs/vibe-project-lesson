...
아래는 `modelcontextprotocol/servers` 저장소에서 제공하는 공식 레퍼런스 서버입니다. MCP 프로토콜의 표준 구현 참조로 사용되며, 현재 AAIF(Agentic AI Foundation) 커뮤니티에서 유지보수합니다.
...
<DataTable
  ...
  data={[
    ...
    { server: 'DBHub', provider: 'Bytebase', description: 'DB 쿼리 실행 (PostgreSQL, MySQL, MariaDB, SQL Server, SQLite 지원)', transport: 'stdio' },
    { server: 'Puppeteer', provider: 'Reference 서버', description: '브라우저 자동화, 스크린샷, 웹 스크래핑 (archived)', transport: 'stdio' },
  ]}
/>
...
<Callout type="warning" title="SSE는 사용 중단됨">
  SSE(Server-Sent Events) 전송 방식은 2025-03-26 MCP 스펙부터 Streamable HTTP로 대체되었습니다 ([MCP Spec](https://modelcontextprotocol.io/specification/2025-03-26)).
...
# Database (DBHub) — PostgreSQL, MySQL, MariaDB, SQL Server, SQLite 쿼리 실행
...
3. **시크릿이 하드코딩된 .mcp.json만 .gitignore에 추가**: 시크릿을 `${VAR}` 형식의 환경 변수 참조로만 두면 `.mcp.json`을 저장소에 커밋하는 것은 팀 공유를 위한 정상적인 패턴입니다(앞의 "팀 공유용" 절 참조). 다만 토큰/키를 파일에 직접 하드코딩한 경우에는 절대 커밋하지 말고 `.gitignore`에 추가한 뒤, `.mcp.json.example`을 템플릿으로 제공하세요.
...
<CodeBlock
  code={`# 시크릿을 파일에 하드코딩한 경우에만 .gitignore에 추가
# (시크릿을 \${VAR} 환경 변수 참조로만 두었다면 .mcp.json은 커밋해도 됨)
.mcp.json

# .mcp.json.example은 커밋 (템플릿으로 제공)
!.mcp.json.example`}
  language="bash"
  filename=".gitignore 설정 (시크릿 하드코딩 시)"
/>
...
# 제공되는 주요 도구 (예시 — 도구 구성과 이름은 서버 버전에 따라 다를 수 있음):
...
# 2단계: 분석 결과 전달 서버들
# Slack은 stdio 서버를 사용 (HTTP 서버는 제한적 파트너 롤아웃 중)
claude mcp add --transport stdio \\
  --env SLACK_BOT_TOKEN=\${SLACK_BOT_TOKEN} \\
  --env SLACK_TEAM_ID=\${SLACK_TEAM_ID} \\
  slack -- npx -y @modelcontextprotocol/server-slack
...
server.tool(
  ...
  async ({ job, count }) => {
    const response = await fetch(
      \`\${JENKINS_URL}/job/\${job}/api/json?tree=builds[number,result,timestamp,duration]{0,\${count}}\`,
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '민감한 설정', values: ['시크릿은 ${VAR} 참조 후 .mcp.json 커밋', '시크릿을 하드코딩한 채 커밋'] },
