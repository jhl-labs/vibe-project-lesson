...
OWASP는 2025년 **Top 10 for LLM Applications**에서 프롬프트 인젝션을 1위,
2026년 **Top 10 for Agentic Applications**에서 에이전트 목표 탈취(Goal Hijack)를 1위로 선정했습니다.
...
2025년 GitHub Copilot의 CVE-2025-53773은 소스 코드 파일이나 GitHub 이슈에 삽입된
...
USENIX Security 2025에서 발표된 연구("We Have a Package for You!")에 따르면,
...
OWASP Top 10 for Agentic Applications(2026)의 **ASI03 Privilege Compromise**는
...
for pattern in "\${PATTERNS[@]}"; do
  if grep -qP "$pattern" "$FILE" 2>/dev/null; then
    echo "❌ 보안 경고: $FILE 에서 시크릿 패턴이 감지되었습니다"
    echo "   패턴: $pattern"
...
<ChapterNav
  prev={{ title: 'End-to-End 워크플로우', path: '/docs/part-5--실전-예제-end-to-end-워크플로우' }}
