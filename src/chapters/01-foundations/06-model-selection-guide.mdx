  ...
  events={[
    {
      ...
      description: 'Claude Code 1.0과 함께 출시. Extended Thinking 도입으로 복잡한 추론 능력 대폭 향상.',
    },
    {
      date: '2025년 9월',
      title: 'Sonnet 4.5 출시',
      description: 'SWE-bench 77.2% 달성. 비용 대비 성능의 새로운 기준 수립. Claude Code 기본 모델로 설정.',
    },
    {
      date: '2025년 10월',
      title: 'Haiku 4.5 출시',
    ...
    },
    {
      date: '2026년 2월',
      title: 'Opus 4.6 출시',
      description: 'Agent Teams 네이티브 지원, 1M 기본 컨텍스트, Adaptive Thinking 도입. 128K 출력 토큰.',
      highlight: true,
    },
  ]}
/>
...
**핵심 특성:**
- **1M 토큰 기본 컨텍스트**: 대규모 코드베이스 전체를 한번에 분석 가능
...
- **200K 컨텍스트 (1M 베타)**: 충분한 프로젝트 컨텍스트 처리
...
Sonnet 4.5는 **Claude Code의 기본 모델**입니다. 성능, 속도, 비용의 균형이 탁월하여 대부분의 일상적인 개발 작업에 최적입니다.
...
<Callout type="tip" title="Sonnet 4.5가 기본 모델인 이유">
  Anthropic이 Claude Code의 기본 모델로 Sonnet 4.5를 선택한 것은 의미가 있습니다.
  "80%의 작업에서 Opus와 체감 차이가 미미하면서도, 비용은 40% 절감"되기 때문입니다.
  모델 선택에 확신이 없다면 Sonnet 4.5로 시작하세요.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    { feature: 'Opus 4.6 (Thinking)', values: ['79.2%', '높음', 'Agent Teams 지원, 128K 출력. 팀 협업 시 시너지'] },
    ...
    { feature: 'GPT-4o', values: ['38.4%', '중간', 'OpenAI 범용 모델'] },
...
<ComparisonTable
  ...
  rows={[
    { feature: 'SWE-bench Verified', values: ['79.2%', '77.2%', '73.3%', '실제 GitHub 이슈 해결'] },
    { feature: 'GPQA Diamond', values: ['84.2%', '65.0%', '56.8%', '대학원 수준 과학/수학'] },
    { feature: 'MATH 500', values: ['96.4%', '88.0%', '82.8%', '수학 문제 해결'] },
    { feature: 'HumanEval', values: ['95.5%', '93.7%', '90.6%', '코드 생성 기본'] },
    { feature: 'MMLU Pro', values: ['84.1%', '78.0%', '69.4%', '다분야 지식'] },
    { feature: 'TAU-bench (Airline)', values: ['62.0%', '53.2%', '47.5%', '에이전트 도구 사용 능력'] },
  ]}
/>
...
<ComparisonTable
  ...
  rows={[
    { feature: '사고 깊이 조절', values: ['수동 (budget_tokens 지정)', '자동 (작업 복잡도 기반)'] },
    { feature: '컨텍스트 윈도우', values: ['200K (1M 베타)', '1M (기본)'] },
    { feature: '최대 출력', values: ['64K 토큰', '128K 토큰'] },
    { feature: 'Effort 파라미터', values: ['미지원', '4단계 (Low/Medium/High/Max)'] },
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'Max', values: ['최대 능력 발휘', '느림', '최고', '아키텍처 설계, 보안 분석, 마이그레이션'] },
  ]}
/>

<CodeBlock
  code={`# API에서 Effort 파라미터 설정 예시
import anthropic
...
response_simple = client.messages.create(
    ...
    max_tokens=16000,
    thinking={"type": "enabled", "budget_tokens": 5000, "effort": "low"},
...
<Callout type="tip" title="Claude Code의 자동 Effort 조절">
  Claude Code에서는 Adaptive Thinking이 자동으로 작동합니다. 간단한 파일 수정에는 Low effort,
  복잡한 아키텍처 설계에는 Max effort가 자동 적용됩니다.
  개발자가 별도로 설정할 필요 없이 최적의 사고 깊이가 선택됩니다.
</Callout>
...
<ComparisonTable
  ...
  rows={[
    { feature: 'Claude Opus 4.6', values: ['79.2%', '1M', 'Agent Teams, Adaptive Thinking, 보안 분석', '단일 벤치마크는 4.5보다 약간 낮음'] },
    { feature: 'Claude Opus 4.5', values: ['80.9%', '200K', '단일 세션 최고 추론, 아키텍처', '멀티 에이전트 미지원'] },
    { feature: 'Claude Sonnet 4.5', values: ['77.2%', '200K', '비용 대비 최고 효율', '최상위 추론에는 한계'] },
    { feature: 'GPT-4o', values: ['38.4%', '128K', '범용 대화, 멀티모달', '코딩 벤치마크 상대적 저조'] },
