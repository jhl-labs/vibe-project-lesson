...
**Zero Data Retention(ZDR)**은 API 요청·응답 데이터를 Anthropic 서버에 **일체 보존하지 않는** 정책입니다. 일반 API 사용 시 안전 및 남용 모니터링 목적으로 30일간 데이터가 보존되지만, ZDR이 적용되면 이 보존 기간이 0일이 됩니다.

<Callout type="warning" title="ZDR은 셀프서비스 기능이 아닙니다">
  ZDR은 환경 변수나 설정 파일로 활성화하는 기능이 아닙니다. Anthropic 영업팀과의 엔터프라이즈 계약을 통해 **조직/API 키 단위로 적용**됩니다. 도입을 원하는 경우 Anthropic 영업팀(sales@anthropic.com)에 문의하세요.
</Callout>

<ComparisonTable
  title="데이터 보존 정책 비교"
  headers={['구분', '일반 API', 'ZDR 적용 API']}
  rows={[
    { feature: '입력/출력 보존', values: ['30일', '보존 안 함'] },
    { feature: '학습 데이터 사용', values: ['no', 'no'] },
    { feature: '적용 방식', values: ['기본 적용', 'Anthropic 영업팀 계약 후 적용'] },
    { feature: '대상', values: ['모든 API 사용자', 'Enterprise 계약 또는 대량 API 고객'] },
  ]}
/>

ZDR이 적용된 API 키는 기존과 동일한 방식으로 Claude Code에서 사용할 수 있습니다. 별도의 클라이언트 측 설정은 필요하지 않습니다.

<CodeBlock
  code={`# ZDR 적용 API 키 사용 — 일반 API 키와 동일한 방식
export ANTHROPIC_API_KEY=sk-ant-...   # ZDR 계약 후 발급받은 키

# 또는 Bedrock/Vertex 경유 시 해당 클라우드의 IAM 인증 사용
# (Bedrock/Vertex는 Anthropic 서버를 경유하지 않으므로 ZDR과 무관)`}
  language="bash"
  filename="ZDR API 키 사용 예시"
/>
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'Microsoft Foundry', values: ['Azure 사용 조직', 'Microsoft Entra ID 연동'] },
  ]}
/>
...
Enterprise 플랜에서는 **SAML 2.0** 및 **OIDC** 기반 SSO를 지원합니다. Okta, Microsoft Entra ID, Auth0 등 주요 IdP와 연동됩니다.
