...
<ComparisonTable
  ...
  rows={[
    { feature: 'Free / Pro / Max', values: ['기본 ON (옵트아웃 가능)', '학습 허용 시 5년 / 거부 시 30일', '개인 설정에서 비활성화'] },
...
Enterprise 플랜에서는 **Zero Data Retention** 모드를 활성화할 수 있습니다. ZDR이 적용되면 Anthropic 서버에 대화 내용이 전혀 보존되지 않습니다. Enterprise 계약에서 별도로 요청해야 합니다.
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'ISO 27001', values: ['yes', '정보보안 관리 체계'] },
    { feature: '전송 암호화', values: ['TLS 1.2+', '모든 API 통신'] },
...
CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1     # 실험 기능 비활성화
