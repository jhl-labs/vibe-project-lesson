...
Claude Code의 **Hooks**는 세션 라이프사이클의 특정 시점에 셸 스크립트, LLM 프롬프트, 또는 에이전트를 자동 실행하는 기능입니다.
...
| `PostToolUseFailure` | 도구 실행 실패 후 | 실패 로깅, 알림 발송 |
...
<Callout type="info" title="Hook 데이터 전달 방식">
  Hook 스크립트는 **stdin으로 JSON 데이터를 수신**합니다. 공통 필드로 `session_id`, `cwd`, `hook_event_name`, `permission_mode` 등이 포함되며, 이벤트별로 `tool_name`, `tool_input`, `prompt` 등 추가 필드가 전달됩니다.
...
<Callout type="tip" title="Hook 종료 코드">
  Hook 스크립트의 종료 코드에 따라 동작이 달라집니다:
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '감사 요구', values: ['전체 이력 5년 보관', '전체 이력 5~10년 보관', '전체 이력 5년 보관'] },
  ...
  events={[
    {
      ...
      description: '2023년 4월 ChatGPT에 소스코드 유출 사건 이후, 5월 1일부터 사내 생성형 AI 사용을 금지하고 내부 AI 플랫폼 개발을 시작했습니다 ([TechCrunch](https://techcrunch.com/2023/05/02/samsung-bans-use-of-generative-ai-tools-like-chatgpt-after-april-internal-data-leak/)).',
      highlight: true
    },
    {
      date: '2024.04',
    ...
    },
    {
      ...
      description: '전 소프트웨어 엔지니어에게 AI 코딩 가이드라인을 배포. AI 생성 코드에 대한 코드 리뷰 기준과 책임 소재를 정의했습니다 ([9to5Google](https://9to5google.com/2025/06/30/google-engineers-ai-code/)).',
    },
    {
      date: '2025.08',
