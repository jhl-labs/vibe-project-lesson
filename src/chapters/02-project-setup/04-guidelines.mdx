...
<Meta title="Part 2: 프로젝트 설정/가이드라인" />
...
권한은 `allow` (자동 허용), `deny` (차단), `ask` (매번 확인) 세 단계로 나뉘며, 평가 순서는 **deny -> ask -> allow**입니다.
...
Claude Code의 **Hooks**는 세션 라이프사이클의 특정 시점에 셸 스크립트, HTTP 엔드포인트, LLM 프롬프트, 또는 에이전트를 자동 실행하는 기능입니다.
가이드라인 준수를 코드 수준에서 강제할 수 있습니다. Hook에는 4가지 유형이 있습니다: **command** (셸 명령), **http** (HTTP POST 요청), **prompt** (단일 LLM 평가), **agent** (도구 접근 가능한 서브에이전트).

### Hook 이벤트 유형

| 이벤트 | 실행 시점 | 활용 예시 |
|--------|----------|----------|
| `SessionStart` | 세션 시작/재개 시 | 개발 컨텍스트 로드, 환경변수 설정 |
| `Setup` | 초기 설정 시 | 환경 초기화 |
| `UserPromptSubmit` | 사용자 프롬프트 제출 전 | 민감 정보 필터링 |
| `PermissionRequest` | 권한 다이얼로그 표시 시 | 자동 허용/거부 처리 |
| `PreToolUse` | 도구 실행 전 | 위험한 명령 차단, 파일 접근 제한 |
| `PostToolUse` | 도구 실행 성공 후 | 린트 자동 실행, 포맷팅 적용 |
| `PostToolUseFailure` | 도구 실행 실패 후 | 실패 로깅, 복구 컨텍스트 제공 |
| `PreCompact` | 대화 압축 직전 | 보존할 컨텍스트 지정 |
| `Notification` | 알림 발생 시 | 슬랙/디스코드 알림 전송 |
| `SubagentStart` | 서브에이전트 시작 시 | 서브에이전트 컨텍스트 주입 |
| `SubagentStop` | 서브에이전트 종료 시 | 서브에이전트 결과 검증 |
| `TeammateIdle` | 에이전트 팀원 유휴 시 | 작업 지속 또는 중단 제어 |
| `Stop` | 에이전트 턴 종료 시 | 자동 테스트 실행, 빌드 검증 |
| `TaskCompleted` | 태스크 완료 표시 시 | 완료 기준 검증 |
| `SessionEnd` | 세션 종료 시 | 정리 작업, 세션 통계 로깅 |

<Callout type="info" title="Hook 데이터 전달 방식">
  **command** Hook은 stdin으로, **http** Hook은 POST body로 JSON 데이터를 수신합니다. 공통 필드로 `session_id`, `cwd`, `hook_event_name`, `permission_mode` 등이 포함되며, 이벤트별로 `tool_name`, `tool_input`, `prompt` 등 추가 필드가 전달됩니다.
  환경변수로는 `$CLAUDE_PROJECT_DIR` (프로젝트 루트), `$CLAUDE_FILE_PATHS` (PostToolUse Write/Edit 시 변경된 파일 경로), `$CLAUDE_SESSION_ID` 등이 지원됩니다.
  **PreToolUse**는 `hookSpecificOutput.permissionDecision`과 `hookSpecificOutput.permissionDecisionReason`으로 도구 입력 수정, 허용/거부/사용자에게 에스컬레이션이 가능합니다 (기존 top-level `decision`/`reason` 필드는 deprecated).
  Hook에서 도구 입력 데이터를 읽으려면 `jq`로 stdin을 파싱하세요: `INPUT=$(cat) && echo "$INPUT" | jq -r '.tool_input.command'`
</Callout>
...
<Callout type="tip" title="Hook 종료 코드와 유형">
  **command** Hook의 종료 코드에 따라 동작이 달라집니다:
  - **종료 코드 0**: 성공. stdout의 JSON 출력이 처리됩니다.
  - **종료 코드 2**: 차단. `PreToolUse`에서는 도구 실행 차단, `UserPromptSubmit`에서는 프롬프트 거부, `Stop`에서는 대화 계속 등 이벤트별로 차단 동작이 다릅니다. stderr 텍스트가 Claude에게 에러 메시지로 전달됩니다.
  - **기타 종료 코드**: 비차단 에러. stderr가 verbose 모드에서만 표시되고 실행은 계속됩니다.

  **http** Hook은 URL로 POST 요청을 보내고, 응답 body의 JSON으로 동일한 결과를 전달합니다.
  **prompt** Hook은 LLM에게 단일 턴 평가를 요청하여 yes/no 판단을 받습니다.
  **agent** Hook은 Read, Grep, Glob 등의 도구를 사용할 수 있는 서브에이전트를 생성합니다.
</Callout>
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_input.command' | grep -qE '(rm -rf /|DROP TABLE|shutdown)' && echo 'BLOCKED: 위험한 명령 감지' >&2 && exit 2 || exit 0"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx eslint --fix $CLAUDE_FILE_PATHS && npx prettier --write $CLAUDE_FILE_PATHS"
          }
        ]
      },
      {
        "matcher": "Write",
        "_comment": "PostToolUse는 도구 실행 후 호출되므로 생성 자체를 막지는 못합니다. 사전 차단은 PreToolUse를 사용하세요. 여기서는 사후 경고(exit 2로 Claude에 피드백)를 보냅니다.",
        "hooks": [
          {
            "type": "command",
            "command": "echo $CLAUDE_FILE_PATHS | grep -qE '\\.(env|pem|key|secret)$' && echo 'WARNING: 민감 파일이 생성되었습니다. 검토 후 제거하세요' >&2 && exit 2 || exit 0"
          }
        ]
      }
    ]
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.prompt' | grep -qiE '(password|secret|api.key|token)\\s*[:=]\\s*[A-Za-z0-9]' && echo 'WARNING: 프롬프트에 민감 정보가 포함된 것으로 보입니다' >&2 && exit 2 || exit 0"
          }
        ]
      }
    ]
...
if [ -n "$COMMAND" ]; then
  ...
  for pattern in "\${DANGEROUS_PATTERNS[@]}"; do
    if echo "$COMMAND" | grep -qi "$pattern"; then
      echo "BLOCKED: 위험한 명령 패턴 감지 - $pattern" >&2
      exit 2
    fi
  done
fi
...
if [ -n "$FILE_PATH" ]; then
  ...
  for pattern in "\${SENSITIVE_PATTERNS[@]}"; do
    if echo "$FILE_PATH" | grep -qi "$pattern"; then
      echo "BLOCKED: 민감 파일 접근 감지 - $FILE_PATH" >&2
      exit 2
    fi
  done
fi
...
if [ -n "$COMMAND" ] || [ -n "$FILE_PATH" ]; then
  SECRET_PATTERNS=(
    ...
    "sk-proj-[a-zA-Z0-9_-]{40,}" # OpenAI API Key (sk-proj- 접두사)
    "ghp_[a-zA-Z0-9]{36,}"      # GitHub Token (36자 이상)
    "sk-ant-api03-[a-zA-Z0-9_-]{40,}" # Anthropic API Key (sk-ant-api03- 접두사)
  )

  CONTENT=$(echo "$TOOL_INPUT" | jq -r '.tool_input.content // .tool_input.command // empty')
  for pattern in "\${SECRET_PATTERNS[@]}"; do
    if echo "$CONTENT" | grep -qE "$pattern"; then
      echo "BLOCKED: 하드코딩된 시크릿 패턴 감지" >&2
      exit 2
    fi
  done
fi
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '감사 요구', values: ['전체 이력 5년 보관', '전체 이력 5~10년 보관', '전체 이력 1년~영구 보관 (유형별 차등)'] },
        ...
        run: |
          ...
          git diff origin/main --name-only | while read file; do
            if [ -f "$file" ]; then
              ...
              if grep -qE '(AKIA|sk-|ghp_|password\s*=\s*"[^"]+)' "$file"; then
...
for file in $STAGED_FILES; do
  if [ -f "$file" ]; then
    # API 키, 토큰 패턴 검사
    if grep -nE '(AKIA[0-9A-Z]{16}|sk-proj-[a-zA-Z0-9_-]{40,}|ghp_[a-zA-Z0-9]{36,}|sk-ant-api03-[a-zA-Z0-9_-]{40,})' "$file"; then
  ...
  events={[
    {
      ...
      description: '2023년 4월 ChatGPT에 소스코드 유출 사건 이후, 5월 1일부터 사내 생성형 AI 사용을 제한했습니다 ([TechCrunch](https://techcrunch.com/2023/05/02/samsung-bans-use-of-generative-ai-tools-like-chatgpt-after-april-internal-data-leak/)).',
      highlight: true
    },
    {
      date: '2024.04',
      title: 'Microsoft - Copilot for Security GA',
      description: '엔터프라이즈 보안 환경에서 AI Copilot 사용을 위한 보안 프레임워크와 함께 Copilot for Security가 정식 출시되었습니다 ([Microsoft Blog](https://www.microsoft.com/en-us/security/blog/2024/03/13/microsoft-copilot-for-security-is-generally-available-on-april-1-2024-with-new-capabilities/)).',
      highlight: true
    },
    {
      date: '2025.06',
      title: 'Google - 내부 AI 코딩 가이드라인 공개',
      description: '전 소프트웨어 엔지니어에게 AI 코딩 가이드라인을 배포. 코드 리뷰, 보안, 유지보수에서 엄격성 유지를 강조했습니다 ([9to5Google](https://9to5google.com/2025/06/30/google-engineers-ai-code/)).',
    },
    {
      date: '2025.08',
      title: 'Anthropic - Claude Code Team/Enterprise 통합',
      description: 'Claude Code가 Team/Enterprise 플랜에 포함되며, Managed Settings, Compliance API, 감사 로그 등 기업용 관리 기능이 추가되었습니다 ([Anthropic Blog](https://www.anthropic.com/news/claude-code-on-team-and-enterprise)).',
      highlight: true
    },
    {
      date: '2025.12',
      title: 'Linux Foundation - Agentic AI Foundation (AAIF) 설립',
      description: 'Anthropic(MCP), OpenAI(AGENTS.md), Block(goose) 등이 참여하여 AI 에이전트 표준화를 위한 재단이 설립되었습니다. AWS, Google, Microsoft 등이 플래티넘 멤버로 참여합니다 ([Linux Foundation](https://www.linuxfoundation.org/press/linux-foundation-announces-the-formation-of-the-agentic-ai-foundation)).',
      highlight: true
    }
  ]}
/>
...
<CodeBlock
  ...
  language="json"
  filename="Enterprise 감사 로그 설정 (개념 설계 예시 — Hook으로 구현)"
/>

<Callout type="info" title="감사 로그 구현 방법">
  Claude Code는 자체적인 감사 로그 설정 JSON을 제공하지 않습니다. 대신 **Hook 시스템**을 활용하여 모든 도구 실행을 기록하는 방식으로 감사 로그를 구현합니다. 아래 Hook 기반 스크립트를 참조하세요.
</Callout>
...
<Callout type="info" title="감사 로그 보관 기간">
  ...
  - **금융**: 최소 5년 (전자금융거래법 제22조)
  - **의료**: 최소 5년~10년 (의료법 시행규칙 제15조, 진료기록부 10년/기타 5년)
  - **공공기관**: 1년~영구 (유형별 차등, 공공기록물 관리에 관한 법률 시행령 제26조)
  - **일반 기업**: 권장 1-3년
...
<ChapterNav
  prev={{ title: '아키텍처', path: '/docs/part-2--프로젝트-설정-아키텍처' }}
  next={{ title: '문서 템플릿', path: '/docs/part-2--프로젝트-설정-문서-템플릿' }}
/>
