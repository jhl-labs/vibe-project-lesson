...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'OpenAI Codex', values: ['AGENTS.md', '프로젝트 루트 + 하위 디렉토리 계층적 로드'] },
    { feature: 'Windsurf', values: ['.windsurf/rules/*.md', '글로벌 규칙 + 워크스페이스 규칙'] },
  ]}
/>
...
| 항목 | 내용 |
|------|------|
| **관리 주체** | Linux Foundation 산하 Agentic AI Foundation (AAIF, 2025년 12월 설립, [Announcement](https://www.linuxfoundation.org/press/linux-foundation-announces-the-formation-of-the-agentic-ai-foundation)) |
| **창립 기여 프로젝트** | Anthropic의 MCP, Block의 goose, OpenAI의 AGENTS.md |
| **플래티넘 멤버** | AWS, Anthropic, Block, Bloomberg, Cloudflare, Google, Microsoft, OpenAI |
| **지원 도구** | Amp, Codex, Cursor, Devin, Factory, Gemini CLI, GitHub Copilot, Jules, VS Code 등 |
| **채택 규모** | 다수의 공개 저장소에서 채택 |
...
| `skills/` | 실행 가능한 스킬 | **자동 발견** + `/스킬명` 호출 | `.claude/skills`에서 자동 로드(마켓플레이스 불필요). 사용자가 `/스킬명`으로 명시 호출하거나, Claude이 컨텍스트에 따라 자동 발견/활용. 각 스킬은 `SKILL.md`를 포함하는 디렉토리 |
...
<MermaidDiagram
  ...
  caption="로드 순서이며 우선순위가 아닙니다 — 조직 관리 정책이 최상위이고, 이후에는 더 구체적인 설정이 일반 설정을 보완·오버라이드합니다(사용자 전역 ~/.claude/CLAUDE.md는 모든 프로젝트에 공통 적용). 조직 관리 정책 경로는 macOS: /Library/Application Support/ClaudeCode/, Linux: /etc/claude-code/, Windows: C:\\Program Files\\ClaudeCode/"
/>
...
| `.claude/skills/*/SKILL.md` | **자동 발견** + `/스킬명` 호출 | `.claude/skills`에서 자동 로드(마켓플레이스 불필요). 명시 호출 또는 Claude이 컨텍스트 기반 자동 활용 |
...
<Callout type="info" title="@ 임포트 동작 방식">
  ...
  임포트된 파일은 재귀적으로 다른 파일을 임포트할 수 있으며, **최대 깊이는 5단계**입니다.
  프로젝트에서 처음 외부 임포트를 사용하면 승인 다이얼로그가 표시됩니다.
  코드 블록 내부의 `@`는 무시됩니다.
...
| 도구/방법 | 생성 대상 | 특징 |
|-----------|----------|------|
| `/init` | CLAUDE.md | 세션 내 슬래시 커맨드. 프로젝트 구조를 분석하여 초기 CLAUDE.md 생성 |
...
<Callout type="tip" title="자동 생성 후 반드시 리뷰하세요">
  `/init`으로 생성된 CLAUDE.md는 프로젝트의 파일 구조를 분석한 결과일 뿐, 비즈니스 규칙이나 팀 컨벤션은 반영하지 못합니다.
