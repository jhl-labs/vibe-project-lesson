...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '메타데이터 지원', values: ['YAML frontmatter (rules)', 'description, globs, alwaysApply', '파일명 기반 (.instructions.md)', 'globs, description 지원', 'YAML frontmatter'] },
    { feature: '범위 지정 (Scoping)', values: ['paths 필드로 glob 패턴', 'globs 필드로 파일 매칭', '파일명에 경로 인코딩', 'globs 패턴 지원', 'glob 패턴 지원'] },
    { feature: '외부 참조', values: ['@path 구문으로 import', '@file 구문', '직접 참조 불가', '직접 참조 불가', '@file 구문'] },
    { feature: '조건부 로드', values: ['path-scoped로 조건부', 'alwaysApply: false로 조건부', '없음 (항상 전체 로드)', 'globs로 조건부', '조건부 지원'] },
