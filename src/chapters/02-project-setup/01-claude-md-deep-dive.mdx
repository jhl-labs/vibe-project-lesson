...
<Meta title="Part 2: 프로젝트 설정/CLAUDE.md 딥다이브" />
...
<MermaidDiagram
  chart={`flowchart TD
    ...
    D --> E[".claude/rules/*.md 스캔"]
    E --> F{paths 매칭?}
    F -->|Yes| G["조건부 규칙 포함"]
    F -->|No| H["건너뛰기"]
    G --> I["@import 구문 해석<br/>(최대 5단계 재귀)"]
    H --> I
    I --> J["CLAUDE.local.md 로드<br/>(개인 설정)"]
...
<FileTree
  title="CLAUDE.md 로딩 계층 (위에서 아래로 로드 순서 — Enterprise 정책이 최상위, 그 외에는 더 구체적인 설정이 일반 설정을 오버라이드)"
  data={[
    {
      name: 'Enterprise 정책',
      type: 'folder',
      description: '조직 전체 (macOS: /Library/Application Support/ClaudeCode/, Linux: /etc/claude-code/, Windows: C:\\Program Files\\ClaudeCode/)',
      children: [
...
<Callout type="info" title="임포트 경로">
  ...
  임포트된 파일은 **재귀적으로 다른 파일을 임포트**할 수 있으며, 최대 깊이는 **5단계**입니다.
  프로젝트에서 처음 외부 임포트를 사용하면 승인 다이얼로그가 표시되며, 거부 시 해당 임포트는 비활성화됩니다.
  코드 블록 내부의 `@`는 무시됩니다.
</Callout>
...
- **기술 스택**: TypeScript, Next.js 15 (App Router), React 19, Tailwind CSS
...
- **기술 스택**: TypeScript, React Native 0.81, Expo SDK 54
...
- **이름**: my-data-pipeline
- **설명**: ETL 데이터 파이프라인
- **기술 스택**: Python 3.13, Apache Airflow, pandas, SQLAlchemy
...
Claude Code는 `~/.claude/projects/<encoded-path>/memory/` 디렉토리에 **영구적인 자동 메모리**를 관리합니다. CLAUDE.md가 **개발자가 Claude에게 주는 지시사항**이라면, MEMORY.md는 **Claude가 프로젝트에 대해 스스로 기록하는 노트**입니다. 대화 간에 유지되는 학습 내용을 기록하여, 세션이 바뀌어도 프로젝트에 대한 이해를 유지합니다.
...
- **자동 기록**: Claude Code가 대화 중 중요한 패턴을 발견하면 MEMORY.md에 자동 추가 (기본 활성화)
- **선택적 기록**: 매 세션마다 기록하는 것이 아니라, 미래 대화에서 유용할 정보만 선별적으로 저장
- **용량 제한**: MEMORY.md는 **200줄까지** 시스템 프롬프트에 포함
- **수동 편집**: `/memory` 커맨드로 시스템 에디터에서 직접 편집 가능
- **# 키**: 대화 중 `#`을 누르면 메모를 메모리 파일(CLAUDE.md 등)에 빠르게 추가
- **로컬 전용**: MEMORY.md는 로컬 머신에만 존재하며 Git에 추적되지 않음
...
# 4. 자동 Compaction 설정
# 모델별 컨텍스트 윈도우 기준으로 일정 비율 사용 시 자동 발동
# 예: 200K 모델 기준 ~167K 토큰(버퍼 ~33K, 약 16.5%)
# 현행 Opus 4.8 / Sonnet 4.6은 1M 토큰 윈도우
# 5. CLAUDE.md에 compaction 보존 지시 추가 가능
# "When compacting, always preserve the full list of modified files"
# 6. 논리적 구간에서 수동 /compact 실행 시 더 좋은 요약 생성`}
  language="text"
...
| `#` 키 | 대화 중 메모리 파일(CLAUDE.md 등)에 지침 빠르게 추가 |
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: 'Hook 유형', values: ['N/A', 'command, http, prompt, agent 4가지', 'N/A'] },
...
<ComparisonTable
  ...
  rows={[
    { feature: '설정 파일명', values: ['CLAUDE.md', 'copilot-instructions.md', '.cursor/rules/', 'AGENTS.md'] },
    { feature: '설정 위치', values: ['프로젝트 루트 / .claude/', '.github/', '.cursor/', '프로젝트 루트'] },
    { feature: '계층 구조', values: ['yes (조직>프로젝트>사용자)', 'yes (조직>저장소)', 'yes', '하위 디렉토리 계층'] },
    { feature: '글로벌 설정', values: ['~/.claude/CLAUDE.md', 'VS Code settings', '~/.cursor/rules/', 'no'] },
    { feature: '조건부 적용', values: ['paths 프론트매터', 'applyTo 프론트매터', 'globs 패턴', 'no'] },
    { feature: '임포트 구문', values: ['@path/to/file (최대 5단계)', 'no', '@file 구문', 'no'] },
    { feature: '개인 설정 분리', values: ['CLAUDE.local.md', 'no', 'no', 'no'] },
    { feature: '자동 메모리', values: ['MEMORY.md', 'no', 'no', 'no'] },
    { feature: '서브트리 설정', values: ['하위 디렉토리 CLAUDE.md', 'no', 'no', '하위 디렉토리 AGENTS.md'] },
    { feature: 'Enterprise 정책', values: ['managed-settings.json', 'no', 'no', 'no'] },
  ]}
/>
...
<ChapterNav
  prev={{ title: '릴리스 히스토리', path: '/docs/part-1--시작하기-릴리스-히스토리' }}
  next={{ title: '프로젝트 지식 구조', path: '/docs/part-2--프로젝트-설정-프로젝트-지식-구조' }}
/>
