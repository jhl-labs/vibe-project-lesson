...
<Timeline
  ...
  events={[
    ...
    {
      date: '2025-05',
      title: 'OAuth 2.1 인증 표준화',
      description: 'MCP 서버의 인증 흐름이 OAuth 2.1 기반으로 표준화. 보안성과 사용 편의성 향상.',
    },
    {
      date: '2025-06',
    ...
    },
    {
      ...
      description: '최신 스펙 버전. Tool Search, 향상된 보안 모델, 성능 최적화 포함.',
      highlight: true,
    },
    {
      date: '2025-12-09',
...
**slack** -- Slack 워크스페이스와 연동하여 메시지 전송, 채널 관리, 대화 검색 등을 수행합니다.

<CodeBlock
  code={`# Slack MCP 서버 설정
...
<MermaidDiagram
  ...
  title="AAIF 구조"
  caption="Linux Foundation 산하에서 MCP와 A2A를 포함한 에이전트 AI 표준을 관리합니다"
/>

<Callout type="tip" title="업계 표준으로 자리잡는 MCP">
  MCP가 Linux Foundation으로 이관됨에 따라, AI 에이전트의 도구 통합이 업계 표준으로 자리잡고 있습니다. OpenAI, Google도 MCP 지원을 발표했으며, A2A(Agent-to-Agent) 프로토콜도 함께 AAIF에 기부되어 에이전트 간 통신 표준도 발전하고 있습니다.
</Callout>
