...
`anthropics/claude-code-action`은 Anthropic이 공식 제공하는 GitHub Action으로, Claude Code의 능력을 GitHub 워크플로우에 직접 통합합니다. PR 리뷰, 이슈 분류, 코드 생성 등 다양한 작업을 자동화하며, 2025년 5월 초기 출시 후 **2025년 9월에 v1.0이 정식 출시**되었습니다 ([GitHub Marketplace](https://github.com/marketplace/actions/claude-code-action)).
...
<CodeBlock
  ...
  with:
    use_azure: "true"
    azure_endpoint: "https://my-endpoint.services.ai.azure.com"
    azure_api_key: \${{ secrets.AZURE_API_KEY }}`}
  language="yaml"
  filename="Azure AI Foundry"
/>
...
<DataTable
  ...
  data={[
    ...
    { option: 'claude_args', required: '아니오', defaultVal: '-', description: 'Claude Code CLI 추가 인자' },
    { option: 'trigger_phrase', required: '아니오', defaultVal: '@claude', description: 'Interactive 모드 트리거 문구' },
    { option: 'assignee_trigger', required: '아니오', defaultVal: 'true', description: '이슈 할당 시 자동 트리거 여부' },
    { option: 'label_trigger', required: '아니오', defaultVal: '-', description: '특정 라벨 추가 시 트리거' },
    { option: 'timeout_minutes', required: '아니오', defaultVal: '60', description: '실행 제한 시간 (분)' },
    { option: 'model', required: '아니오', defaultVal: 'Sonnet (최신)', description: '사용할 Claude 모델 (claude-sonnet-4-*, claude-opus-4-* 등)' },
    { option: 'max_turns', required: '아니오', defaultVal: '-', description: '최대 에이전트 턴 수' },
    { option: 'allowed_tools', required: '아니오', defaultVal: '-', description: '허용할 도구 목록 (쉼표 구분)' },
    { option: 'disallowed_tools', required: '아니오', defaultVal: '-', description: '차단할 도구 목록 (쉼표 구분)' },
    { option: 'custom_instructions', required: '아니오', defaultVal: '-', description: 'CLAUDE.md 외 추가 지시사항' },
    ...
    { option: 'use_azure', required: '아니오', defaultVal: 'false', description: 'Azure AI Foundry 사용 여부' },
  ]}
/>
...
<CodeBlock
  ...
  with:
    ...
    claude_args: >-
      --max-turns 10
      --model claude-sonnet-4-20250514  # 또는 claude-opus-4-* 등
...
| `structured_output` | 구조화된 JSON 출력 (structured_output_prompt 사용 시) |
      ...
      - id: claude
        ...
        with:
          ...
          structured_output_prompt: |
            ...
            {"complexity": "low|medium|high", "issues": [...]}
    ...
    steps:
      ...
      - uses: anthropics/claude-code-action@v1
        with:
          ...
          max_turns: "5"
          allowed_tools: "Read,Bash(gh issue list),Bash(gh issue view)"`}
  language="yaml"
...
- **timeout_minutes**: 적절한 타임아웃 설정으로 비정상 실행 방지
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: '구조화 출력', values: ['없음', 'structured_output_prompt (신규)'] },
